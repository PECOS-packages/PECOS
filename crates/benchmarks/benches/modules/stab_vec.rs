...
use pecos_simulators::{
    ArbitraryRotationGateable, CHForm, CliffordGateable, QuantumSimulator, SparseStab, StabVec,
};
...
fn bench_rz_term_growth<M: Measurement>(c: &mut Criterion<M>) {
    ...
    for &num_rz in &[1, 2, 4, 6, 8] {
        group.bench_with_input(
            BenchmarkId::new("StabVec", format!("{num_rz}_rz")),
            &num_rz,
            |b, &nrz| {
                b.iter(|| {
                    let mut sim = StabVec::new_with_seed(num_qubits, 42);
...
fn bench_measurement<M: Measurement>(c: &mut Criterion<M>) {
    let mut group = c.benchmark_group("StabVec Measurement");
    group.sample_size(20);
    ...
    for &num_rz in &[1, 2, 3, 4] {
        ...
        group.bench_with_input(
            ...
            |b, &nrz| {
                b.iter(|| {
                    let mut sim = StabVec::new_with_seed(4, 42);
                    for q in 0..4 {
                        sim.h(&[QubitId(q)]);
                    }
                    for i in 0..nrz {
...
fn bench_measurement_only<M: Measurement>(c: &mut Criterion<M>) {
    let mut group = c.benchmark_group("StabVec Measurement Only");
    group.sample_size(10);
    ...
    for &num_qubits in &[4, 8, 14, 18, 22] {
        group.bench_with_input(
            ...
            |b, &nq| {
                ...
                let mut template = StabVec::new_with_seed(nq, 42);
                for q in 0..nq {
...
fn bench_realistic_circuit<M: Measurement>(c: &mut Criterion<M>) {
    ...
    for &num_rz in &[2, 4, 6] {
        ...
        group.bench_with_input(
            ...
            &num_rz,
            |b, &nrz| {
                let mut template = StabVec::new_with_seed(num_qubits, 42);
