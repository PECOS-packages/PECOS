...
detector(2, 0, 0, 2) D2
    ...
    let detection_events = vec![0b0000_0011_u8]; // D0 and D1 triggered
    let result = decoder.decode_detection_events(&detection_events);
    assert!(
        result.is_ok(),
...
detector(2, 0, 0, 2) D2
    ...
    let test_patterns = vec![
        0b0000_0000_u8, // No detections
        0b0000_0001_u8, // D0 only
        0b0000_0010_u8, // D1 only
        0b0000_0011_u8, // D0 and D1
        0b0000_0110_u8, // D1 and D2
        0b0000_0101_u8, // D0 and D2
    ];
    ...
    for _ in 0..10 {
        for &pattern in &test_patterns {
            ...
            if let Ok(_result) = decoder.decode_detection_events(&detection_events) {
                decode_count += 1;
...
fn generate_color_code_dem(distance: usize, error_rate: f64) -> String {
    ...
    writeln!(dem, "error({error_rate}) D0 L0").unwrap();
    ...
    for i in 0..distance {
        for j in 0..distance {
            ...
            writeln!(dem, "detector({i}, {j}, 0, {color_basis}) D{idx}").unwrap();
        }
    }
...
fn generate_multi_round_dem(rounds: usize) -> String {
    ...
    for r in 0..rounds {
        ...
        writeln!(dem, "error(0.01) D{} D{}", r * 3, r * 3 + 1).unwrap();
        writeln!(dem, "error(0.01) D{} D{} L0", r * 3 + 1, r * 3 + 2).unwrap();

        // Add detectors for this round
        for i in 0..3 {
            writeln!(dem, "detector({}, {}, {}, {}) D{}", i, 0, r, i, r * 3 + i).unwrap();
        }
    }
