...
fn test_measure_leaked_basic_functionality() {
    ...
    assert!(outcomes[0] <= 1, "MeasureLeaked without leakage should return 0 or 1");
...
fn test_measure_leaked_with_general_noise_model() {
    ...
    assert_eq!(outcomes[0], 1, "Regular Measure on leaked qubit should return 1");
    assert_eq!(outcomes[1], 0, "MeasureLeaked on non-leaked qubit should return 0");
    assert_eq!(outcomes[2], 2, "MeasureLeaked on leaked qubit should return 2");
}
...
fn test_measure_leaked_preserves_quantum_state() {
    ...
    for _ in 0..runs {
        ...
        assert_eq!(outcomes.len(), 2);
        assert!(outcomes[0] <= 1 && outcomes[1] <= 1, "No leakage should occur");
    ...
    assert_eq!(same_results, runs, "Bell state measurements should be perfectly correlated");
}
...
fn test_measure_leaked_sequential_measurements() {
    ...
    assert_eq!(outcomes1[0], 2, "First MeasureLeaked should return 2 for leaked qubit");
    ...
    assert_eq!(outcomes2[0], 2, "Second MeasureLeaked should still return 2");
    ...
    assert_eq!(outcomes3[0], 1, "Regular Measure should return 1 for leaked qubit");
}
...
fn test_measure_leaked_with_prep_unleaks() {
    ...
    assert_eq!(outcomes2[0], 0, "MeasureLeaked after prep should return 0 (unleaked)");
}