...
//! Integration tests for `StabVecEngine` via `ByteMessage`.
//!
//! Tests the full `ByteMessage` -> `Engine::process()` -> `StabVec` path.

use pecos_core::Angle64;
use pecos_engines::Engine;
use pecos_engines::byte_message::ByteMessageBuilder;
use pecos_engines::quantum::{StabVecEngine, StateVecEngine};
...
fn run(num_qubits: usize, build: impl FnOnce(&mut ByteMessageBuilder)) -> Vec<u32> {
    let mut engine = StabVecEngine::with_seed(num_qubits, 42);
...
#[test]
fn engine_reset() {
    let mut engine = StabVecEngine::with_seed(1, 42);
...
fn deterministic_seed() {
    ...
    let make = |seed: u64| -> Vec<u32> {
        let mut engine = StabVecEngine::with_seed(2, seed);
...
fn builder_pattern() {
    ...
    use pecos_engines::stab_vec;

    let mut b = stab_vec().qubits(2);
...
fn round_trip_statistical_comparison() {
    // Run the same circuit on StabVecEngine and StateVecEngine many times,
    ...
    for seed in 0..num_shots as u64 {
        // StabVec engine
        {
            let mut engine = StabVecEngine::with_seed(num_qubits, seed);
            let mut b = ByteMessageBuilder::new();
            let _ = b.for_quantum_operations();
            build_circuit(&mut b);
    ...
    // 2. Pruning in StabVec introduces tiny errors
    ...
    for i in 0..num_outcomes {
        ...
        assert!(
            ...
            "Outcome {i:03b}: StabVec={crz_prob:.4}, StateVec={sv_prob:.4}, diff={:.4}, tol={tolerance:.4}",
...
fn round_trip_deterministic_clifford_only() {
    ...
    for seed in 0..100u64 {
        let mut crz = StabVecEngine::with_seed(3, seed);
