...
fn recipe_quick_start() {
    ...
    let error_rate = run_with_noise(&commands, || depolarizing_with_measurement(0.01, 0.05, 0.02), 1000);
    println!("  depolarizing_with_measurement: {:.1}% error rate", error_rate * 100.0);

    // Method 2: Using the builder
    let error_rate = run_with_noise(&commands, || {
...
fn recipe_sim_neo_integration() {
    ...
    let error_rate = results.outcomes.iter()
    ...
    let error_rate = results.outcomes.iter()
        .filter(|o| !o.get_bit(pecos_core::QubitId(0)).unwrap_or(true))
        .count() as f64 / 1000.0;
    println!("  .depolarizing() convenience: {:.1}% error rate", error_rate * 100.0);
    ...
    let err1 = results1.outcomes.iter()
        .filter(|o| !o.get_bit(pecos_core::QubitId(0)).unwrap_or(true))
        .count();
    let err2 = results2.outcomes.iter()
        .filter(|o| !o.get_bit(pecos_core::QubitId(0)).unwrap_or(true))
        .count();
    println!("  Reusable sim (seed 100): {} errors, (seed 200): {} errors", err1, err2);
...
fn recipe_measurement_noise() {
    ...
    println!("  Symmetric (5%/5%): {:.1}% flipped to 1", error_rate * 100.0);

    // Asymmetric: easier to flip 0->1 than 1->0
    let error_rate = run_with_noise(&commands, || measurement_only(0.10, 0.02), 1000);
    println!("  Asymmetric (10%/2%): {:.1}% flipped to 1", error_rate * 100.0);
...
fn recipe_leakage_model() {
    ...
    let commands = CommandBuilder::new()
        .prep(0)
        .h(0).h(0).h(0).h(0).h(0)  // 5 Hadamard gates
        .measure(0)
        .build();
    ...
    println!("  with_leakage pattern: {:.1}% error rate", error_rate * 100.0);

    // Custom leakage with flow primitives
    let error_rate = run_with_noise(&commands, || {
        let sq_noise = seq![
            skip_if_leaked(),  // Leaked qubits skip the gate
            prob(0.1,          // 10% fault probability
                when_leaked(
    ...
    println!("  Custom flow primitives: {:.1}% error rate", error_rate * 100.0);
...
fn recipe_custom_flow() {
    ...
    let error_rate = run_with_noise_2q(&commands, || {
        let tq_noise = seq![
            skip_if_leaked(),
            prob(0.05,  // 5% error rate
                sample![
    ...
    println!("  Z-biased two-qubit noise: {:.1}% error rate", error_rate * 100.0);
...
fn recipe_crosstalk() {
    ...
    let commands = CommandBuilder::new()
        ...
        .prep(1)
        .prep(2)
        .measure(1)  // Measure middle qubit
        .measure(0)  // Check if neighbors were affected
        .measure(2)
        .build();
    ...
    for seed in 0..1000 {
        ...
        if outcomes.get(pecos_core::QubitId(2)).map(|o| o.outcome) == Some(true) {
            neighbor_errors += 1;
        }
    }

    println!("  Chain crosstalk: {:.1}% neighbor errors (2000 opportunities)",
        neighbor_errors as f64 / 20.0);
...
fn recipe_realistic_device() {
    ...
    let same_outcome = count_bell_correlation(&commands, || {
    ...
    println!("  Realistic device: {:.1}% same outcome (expect ~95-99% with noise)",
    ...
    println!("  Surface code noise (p=0.1%): {:.1}% same outcome",
        same_outcome as f64 / 10.0);
