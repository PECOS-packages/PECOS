...
fn example_depolarizing_noise() {
    ...
    for error_rate in [0.0, 0.05, 0.10, 0.20] {
        ...
        let measured_rate = errors as f64 / shots as f64;
        println!(
...
fn example_asymmetric_measurement() {
    ...
    let commands_0 = CommandBuilder::new()
        .prep(0)
        .measure(0)
        .build();

    // Test measurement of |1⟩
    let commands_1 = CommandBuilder::new()
        .prep(0)
        .x(0)
        .measure(0)
        .build();
    ...
    println!("  Configured: p(0→1)={:.0}%, p(1→0)={:.0}%", p_0_to_1 * 100.0, p_1_to_0 * 100.0);
    println!("  Measured |0⟩: {:.1}% flipped to |1⟩", flips_0 as f64 / shots as f64 * 100.0);
    println!("  Measured |1⟩: {:.1}% flipped to |0⟩", flips_1 as f64 / shots as f64 * 100.0);
    println!();
}
...
fn example_multi_channel() {
    ...
    println!("    Correlated (|00⟩ + |11⟩): {:.1}%", correlated as f64 / shots as f64 * 100.0);
    println!("    Anti-correlated (|01⟩ + |10⟩): {:.1}%", anti_correlated as f64 / shots as f64 * 100.0);
    println!();
}
...
fn example_builder_api() {
    ...
    for (outcome, count) in &counts {
        println!("    |{}⟩: {:.1}%", outcome, *count as f64 / shots as f64 * 100.0);
    }
    println!();
}
...
fn example_idle_noise() {
    ...
    println!("  Measured decoherence error rate: {:.1}%", error_rate * 100.0);
...
fn example_z_biased_noise() {
    ...
    let z_biased_channel = SingleQubitChannel::depolarizing(0.10)
        .with_pauli_weights(PauliWeights::z_biased(0.9));
    ...
    println!("    Uniform depolarizing: {:.1}% |1⟩", uniform_errors as f64 / shots as f64 * 100.0);
    println!("    Z-biased (90% Z): {:.1}% |1⟩", z_biased_errors as f64 / shots as f64 * 100.0);
