# PHIR-JSON Versioning Strategy

This document outlines the strategy for handling multiple versions of the PHIR-JSON (PECOS High-level Intermediate
Representation JSON format) specification in the codebase.

## Overview

PHIR-JSON is a versioned specification, with each version potentially introducing new features, changes, or improvements. To
...
At runtime, the crate detects which version of PHIR-JSON is being used by examining the "version" field in the input JSON:
...
pub fn detect_version(json: &str) -> Result<PhirJsonVersion, PecosError> {
    ...
    if let Some(version) = value.get("version").and_then(|v| v.as_str()) {
        match version {
            ...
            "0.2.0" => Ok(PhirJsonVersion::V0_2),
            _ => Err(PecosError::Input(format!("Unsupported PHIR-JSON version: {}", version)))
        }
    } else {
        Err(PecosError::Input("Missing version field in PHIR-JSON program".into()))
    }
}
```
...
```rust
// Automatically detect and handle the version based on the PHIR-JSON program
let engine = setup_phir_json_engine(path_to_phir_file)?;
```
...
When adding a new version of the PHIR-JSON specification:
...
3. **Update version detection**:
   - Add the new version to the `PhirJsonVersion` enum
...
This versioning strategy allows PHIR-JSON to evolve while maintaining backward compatibility when needed. By isolating each
