use crate::v0_1::environment::{Environment, TypedValue, BoolBit};
...
pub trait EnhancedResultHandling {
    ...
    fn get_result_bits(&self, var_name: &str, bit_indices: &[usize]) -> Result<Vec<BoolBit>, PecosError>;
    
    /// Convert a variable to a bit string
    fn get_result_as_bit_string(&self, var_name: &str, width: Option<usize>) -> Result<String, PecosError>;
...
impl ResultUtils {
    ...
    pub fn bits_to_int_with_indices(bits: &[BoolBit], indices: &[usize]) -> u64 {
        ...
        for (i, (&bit, &idx)) in bits.iter().zip(indices.iter()).enumerate() {
            if bit.0 {
                result |= 1 << idx;
            }
        }
    ...
    pub fn combine_bits(env: &Environment, var_name: &str, bit_indices: &[usize]) -> Result<u64, PecosError> {
