...
fn download_and_extract_stim(out_dir: &Path) -> Result<()> {
    let info = stim_download_info("chromobius");
    let tar_gz = download_cached(&info)?;
    extract_archive(&tar_gz, out_dir, Some("stim_shared"))?;

    if std::env::var("PECOS_VERBOSE_BUILD").is_ok() {
        println!("cargo:warning=Shared Stim source downloaded and extracted");
    }
    Ok(())
}

/// Get the essential Stim source files needed for Chromobius
pub fn collect_stim_sources_chromobius(stim_src_dir: &Path) -> Result<Vec<PathBuf>> {
