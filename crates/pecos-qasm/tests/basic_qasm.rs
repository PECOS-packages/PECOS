...
fn test_bell_qasm() {
    ...
    assert!(has_three, "Expected at least one '3' outcome but found none");
}
...
fn test_x_qasm() {
    ...
    assert!(results.contains_key("d"), "Results should contain 'd' register");
    assert_eq!(results["d"].len(), 10, "Expected 10 measurement results");

    let expected = vec![1u32; 10];
    assert_eq!(results["d"], expected, "Expected all measurement results to be 1");
}
...
fn test_arbitrary_register_names() {
    ...
    assert!(results.contains_key("result"), "Results should contain 'result' register");

    // Assert that "result" has exactly 10 elements
    assert_eq!(results["result"].len(), 10, "Expected 10 measurement results");
...
fn test_flips_multi_reg_qasm() {
    ...
    assert!(results.contains_key("c"), "Results should contain 'c' register");
    assert!(results.contains_key("d"), "Results should contain 'd' register");
    ...
    assert_eq!(results["c"], expected, "Expected all measurement results to be 3");

    let expected = vec![4; 10];
    assert_eq!(results["d"], expected, "Expected all measurement results to be 4");
}
...
fn test_basic_arthmetic_qasm() {
    ...
    assert!(results.contains_key("a"), "Results should contain 'a' register");
    assert!(results.contains_key("b"), "Results should contain 'b' register");

    assert_eq!(results["a"].len(), 10, "Expected 10 measurement results for 'a'");
    assert_eq!(results["b"].len(), 10, "Expected 10 measurement results for 'b'");
    ...
    assert_eq!(results["a"], expected_a, "Expected all 'a' results to be 3 (1+2)");
    ...
    assert_eq!(results["b"], expected_b, "Expected all 'b' results to be 1 at bit 0");
}
...
fn test_defaults_qasm() {
    ...
    assert!(results.contains_key("a"), "Results should contain 'a' register");
    assert!(results.contains_key("b"), "Results should contain 'b' register");
    assert!(results.contains_key("m"), "Results should contain 'm' register");
