...
fn test_comprehensive_qasm_program() {
    ...
    assert!(program.quantum_registers.contains_key("q"), "Should have register q");
    assert_eq!(program.quantum_registers["q"].len(), 4, "Register q should have 4 qubits");
}
...
fn test_qasm_with_comments_and_expressions() {
    ...
    let program = QASMParser::parse_str(qasm).expect("Failed to parse QASM with comments and expressions");
...
fn test_mathematical_constants_and_functions() {
    ...
    let program = QASMParser::parse_str(qasm).expect("Failed to parse QASM with mathematical constants");
    
    // Verify mathematical expressions are handled correctly
    assert!(!program.operations.is_empty(), "Should have operations with mathematical expressions");
}