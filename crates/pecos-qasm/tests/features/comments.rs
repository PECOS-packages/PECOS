...
fn test_international_comments() {
    ...
    match result {
        Ok(program) => {
            ...
            assert_eq!(program.quantum_registers.len(), 1, "Expected 1 quantum register");
            assert_eq!(program.classical_registers.len(), 1, "Expected 1 classical register");
...
fn test_inline_comments_with_emojis() {
    ...
    match result {
        Ok(program) => {
            ...
            let operations: Vec<String> = program.operations.iter()
            ...
            assert_eq!(operations, vec!["H", "CX", "H"], "Expected H, CX, H sequence");
        }
        Err(e) => {
