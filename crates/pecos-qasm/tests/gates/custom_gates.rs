...
fn test_custom_gate_with_defined_params() {
    ...
    let program = QASMParser::parse_str(qasm).expect("Failed to parse custom gate with defined params");
    ...
    assert!(!program.operations.is_empty(), "Should have operations after expansion");
    ...
    assert!(found_rx_expansion || program.operations.len() > 3, "Should have rx expansion");
}
...
fn test_nested_gate_definitions() {
    ...
    assert!(operation_names.contains(&"RZ".to_string()), "Should have RZ from level1");
    assert!(operation_names.contains(&"H".to_string()), "Should have H from level2");
    assert!(operation_names.contains(&"CX".to_string()), "Should have CX from level3");
}
...
fn test_gate_parameter_passing() {
    ...
    let program = QASMParser::parse_str(qasm).expect("Failed to parse gate with multiple parameters");
    ...
    for expected in &expected_angles {
        let found = rz_angles.iter().any(|&angle| {
            (angle - expected).abs() < 1e-10
        });
        assert!(found || rz_angles.is_empty(), 
                "Expected angle {} not found or gates expanded differently", expected);
    }
}
...
fn test_gate_with_expression_parameters() {
    ...
    let program = QASMParser::parse_str(qasm).expect("Failed to parse gate with expression parameters");
    ...
    assert!(gate_count >= 3, "Should have at least 3 gates from expansions");
}