...
fn test_crz_expansion() {
    ...
    match result {
        Ok(program) => {
            println!("CRZ expansion resulted in {} operations", program.operations.len());
            
            // crz(theta) expands to: rz(theta/2) b; cx a,b; rz(-theta/2) b; cx a,b;
            assert_eq!(program.operations.len(), 4, "CRZ should expand to 4 operations");
            ...
            match &program.operations[0] {
                Operation::Gate { name, parameters, qubits } => {
                    ...
                    assert!((parameters[0] - std::f64::consts::PI / 4.0).abs() < 1e-10, 
...
fn test_crx_expansion() {
    ...
    match result {
        Ok(program) => {
            println!("CRX expansion resulted in {} operations", program.operations.len());
            ...
            let gate_types: Vec<&str> = program.operations.iter()
            ...
            assert!(gate_types.contains(&"H"), "CRX should contain H gates from RY expansion");
            assert!(gate_types.contains(&"RZ"), "CRX should contain RZ gates from RY expansion");
...
fn test_cry_expansion() {
    ...
    match result {
        Ok(program) => {
            println!("CRY expansion resulted in {} operations", program.operations.len());
            ...
            assert!(program.operations.len() > 4, "CRY should expand to more than 4 operations due to ry expansion");
            ...
            println!("CRY gate counts - CX: {}, H: {}, RZ: {}", cx_count, h_count, rz_count);
