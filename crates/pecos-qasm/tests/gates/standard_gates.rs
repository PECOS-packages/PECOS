...
fn test_comprehensive_gate_operations() {
    ...
    assert!(program.operations.len() > 0, "Should have operations");
    
    // Verify that important gates are defined (either natively or through qelib1)
    assert!(
        program.gate_definitions.contains_key("rx") || 
        program.operations.iter().any(|op| matches!(op, pecos_qasm::Operation::Gate { name, .. } if name == "rx")),
        "rx gate should be available"
    );
    
    assert!(
        program.gate_definitions.contains_key("rxx") || 
        program.operations.iter().any(|op| matches!(op, pecos_qasm::Operation::Gate { name, .. } if name == "rxx")),
        "rxx gate should be available"
    );
    
    assert!(
        program.gate_definitions.contains_key("rzz") || 
        program.operations.iter().any(|op| matches!(op, pecos_qasm::Operation::Gate { name, .. } if name == "rzz")),
        "rzz gate should be available"
    );
    
    assert!(
        program.gate_definitions.contains_key("cz") || 
        program.operations.iter().any(|op| matches!(op, pecos_qasm::Operation::Gate { name, .. } if name == "cz")),
        "cz gate should be available"
    );
    
    assert!(
        program.gate_definitions.contains_key("ccx") || 
        program.operations.iter().any(|op| matches!(op, pecos_qasm::Operation::Gate { name, .. } if name == "ccx")),
        "ccx gate should be available"
    );
    
    assert!(
        program.gate_definitions.contains_key("u3") || 
        program.operations.iter().any(|op| matches!(op, pecos_qasm::Operation::Gate { name, .. } if name == "u3")),
        "u3 gate should be available"
    );
    
    assert!(
        program.gate_definitions.contains_key("cu1") || 
        program.operations.iter().any(|op| matches!(op, pecos_qasm::Operation::Gate { name, .. } if name == "cu1")),
...
fn test_mathematical_expressions_in_parameters() {
    ...
    let program = QASMParser::parse_str(qasm).expect("Failed to parse QASM with mathematical expressions");
    
    // Just verify it parses without errors
    assert!(!program.operations.is_empty(), "Should have parsed operations with mathematical expressions");
}
...
fn test_comments_and_whitespace() {
    ...
    assert!(!program.operations.is_empty(), "Should have parsed operations despite comments");
}