...
fn test_hqslib1_rzz_sequence() {
    ...
    let rzz_gates: Vec<_> = program.operations.iter()
    ...
    for (i, (params, qubits)) in rzz_gates.iter().enumerate() {
        assert_eq!(params.len(), 1, "RZZ gate {} should have 1 parameter", i);
        assert_eq!(qubits.len(), 2, "RZZ gate {} should have 2 qubits", i);
        assert_eq!(qubits[0], 0, "RZZ gate {} first qubit should be q[0]", i);
        assert_eq!(qubits[1], 1, "RZZ gate {} second qubit should be q[1]", i);
    }
    ...
    let expected_params = vec![
        ...
        -0.2999999999999998 * std::f64::consts::PI,
        0.6 * std::f64::consts::PI,
        1.0 * std::f64::consts::PI,
    ];
    ...
    for (i, ((params, _), expected)) in rzz_gates.iter().zip(expected_params.iter()).enumerate() {
        ...
        assert!(delta < 1e-10, "RZZ gate {} parameter mismatch: expected {}, got {}", i, expected, params[0]);
    }
}
...
fn test_rzz_with_negative_parameters() {
    ...
    let program = QASMParser::parse_str(qasm).expect("Failed to parse QASM with negative RZZ parameters");
    
    let rzz_parameters: Vec<f64> = program.operations.iter()
        .filter_map(|op| match op {
    ...
    assert!(rzz_parameters[0] < 0.0, "First parameter should be negative");
    assert!(rzz_parameters[1] < 0.0, "Second parameter should be negative");
    assert!(rzz_parameters[2] < 0.0, "Third parameter should be negative");
...
fn test_rzz_mixed_with_other_gates() {
    ...
    let h_count = program.operations.iter()
        ...
        .count();
    let rzz_count = program.operations.iter()
    ...
    let cx_count = program.operations.iter()
        ...
        .count();
    let measure_count = program.operations.iter()
    ...
    let gate_sequence: Vec<&str> = program.operations.iter()
        .filter_map(|op| match op {
