...
fn test_p_zero_gate_compiles() {
    ...
    let _messages = engine.generate_commands().expect("p(0) gate should compile");
...
fn test_gate_definitions_updated() {
    ...
    if let Some(p_def) = program.gate_definitions.get("p") {
        ...
        println!("p gate correctly defined with {} operations", p_def.body.len());
    ...
    if let Some(u_def) = program.gate_definitions.get("u") {
        ...
        println!("u gate correctly defined with {} operations", u_def.body.len());
...
fn test_p_gate_expansion() {
    ...
    assert_eq!(program.operations.len(), 1, "Should have 1 expanded operation");
    ...
    if let Some(op) = program.operations.first() {
        match op {
            Operation::Gate { name, parameters, .. } => {
                ...
                assert!((parameters[0] - std::f64::consts::PI / 2.0).abs() < 0.0001, 
...
fn test_identity_operations() {
    ...
    assert!(program.operations.len() >= 2, "Should have at least 2 operations");
}