...
fn test_bell_state_simulation() {
    ...
    assert!(count_00 > 400 && count_00 < 600, "Expected ~500 |00⟩ states, got {}", count_00);
    assert!(count_11 > 400 && count_11 < 600, "Expected ~500 |11⟩ states, got {}", count_11);
}
...
fn test_ghz_state_simulation() {
    ...
    assert!(count_000 > 400 && count_000 < 600, "Expected ~500 |000⟩ states, got {}", count_000);
    assert!(count_111 > 400 && count_111 < 600, "Expected ~500 |111⟩ states, got {}", count_111);
}
...
fn test_phase_kickback() {
    ...
    for &value in c_values {
        ...
        assert_eq!(control_bit, 1, "Control qubit should always be |1⟩ after phase kickback");
