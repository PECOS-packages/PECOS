...
fn test_mixed_gates_circuit() {
    ...
    assert!(gate_count > 7, "Should have more than 7 operations after expansion");
...
fn test_angle_precision() {
    ...
    assert!(!rz_angles.is_empty(), "Should have RZ gates after expansion");
    ...
    for expected in &expected_angles {
        ...
        assert!(found, "Expected angle {} not found in RZ gates", expected);
    }
}
...
fn test_gate_sequence() {
    ...
    assert!(q3_operations.len() > 5, "Qubit 3 should have multiple operations after expansion");
    
    // Check that the operations include expected gate types
    assert!(q3_operations.iter().any(|g| g == "RZ"), "Should have RZ gates on qubit 3");
    assert!(q3_operations.iter().any(|g| g == "CX"), "Should have CX gates on qubit 3");
    assert!(q3_operations.iter().any(|g| g == "H"), "Should have H gates from expansions");
}
...
fn test_two_qubit_gates() {
    ...
    let cx_gates: Vec<_> = two_qubit_gates.iter()
    ...
    assert!(cx_gates.iter().any(|(_, q1, q2)| *q1 == 0 && *q2 == 3),
            "Should have CX between qubits 0 and 3");
    assert!(cx_gates.iter().any(|(_, q1, q2)| *q1 == 0 && *q2 == 5),
