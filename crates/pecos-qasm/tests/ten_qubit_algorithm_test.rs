...
fn test_ten_qubit_quantum_algorithm() {
    ...
    assert_eq!(program.quantum_registers["q"].len(), 10, "Should have 10 qubits");
    assert_eq!(program.classical_registers["c"], 10, "Should have 10 classical bits");
}
...
fn test_cz_gate_dense_connectivity() {
    ...
    // Count CZ operations
    let cz_count = program.operations.iter()
        .filter(|op| matches!(op, pecos_qasm::Operation::Gate { name, .. } if name == "cz"))
        .count();
    
    assert_eq!(cz_count, 0, "CZ gates should be expanded and not appear directly");
    
    // But we should have many operations from the expansions
    assert!(!program.operations.is_empty(), "Should have operations from CZ expansions");
}
...
fn test_precision_angle_values() {
    ...
    assert!(!program.operations.is_empty(), "Should have operations with precise angles");
}
...
fn test_phase_pattern_structure() {
    ...
    assert!(!program.operations.is_empty(), "Should have phase pattern operations");
}