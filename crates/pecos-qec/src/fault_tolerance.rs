...
//!
//! This module provides multiple approaches to analyzing fault tolerance, from
//! code-level analysis to circuit-level verification.
//!
//! # Fault Classification
//!
//! Individual faults are classified into three categories:
//!
//! | Class | Syndrome | Logical Error | Meaning |
//! |-------|----------|---------------|---------|
//! | Stabilizer | None | No | Equivalent to stabilizer element. Harmless. |
//! | Detectable | Non-trivial | Varies | Triggers syndrome. Decoder can attempt correction. |
//! | Undetectable Logical | None | Yes | Logical failure with no warning. Fatal. |
//!
//! # Analysis Approaches
//!
...
//! assert_eq!(analysis.total_errors, 9); // 3 qubits * 3 Pauli types
//! ```
//!
//! Best for: Analyzing code properties independent of circuit implementation.
//! Handles dynamic circuits naturally since stabilizer state persists regardless
//! of measurement.
//!
//! ## Level 2: Pauli Propagation Analysis ([`PauliPropChecker`])
//!
//! Propagates Pauli errors through a circuit to determine syndrome and logical
//! effects. Circuit-aware analysis.
//!
//! ```
//! use pecos_qec::PauliPropChecker;
//! use pecos_quantum::TickCircuit;
//!
//! // Build a simple syndrome extraction circuit for 3-qubit code
...
//! println!("Total faults analyzed: {}", results.len());
//! ```
//!
//! Best for: Verifying specific circuit implementations are fault-tolerant.
//!
//! ## Level 3: Syndrome History Analysis
//!
//! Tracks syndromes across multiple measurement rounds. Essential for multi-round
//! QEC where faults in round k may only be detected in round k+1.
//!
//! ```
//! use pecos_qec::PauliPropChecker;
//! use pecos_quantum::TickCircuit;
//!
//! // Two-round syndrome extraction circuit
//! let mut circuit = TickCircuit::new();
...
//! ```
//!
//! Best for: Multi-round QEC protocols where decoder uses syndrome history.
//!
//! # Fault Tolerance Verdict
//!
//! For weight-t faults, fault tolerance is binary:
//!
//! - **Fault Tolerant**: No undetectable logical errors, no ambiguous syndromes
//! - **Not Fault Tolerant**: Undetectable logical errors exist OR same syndrome
//!   can lead to different logical effects (ambiguous)
//!
//! # Submodules
//!
