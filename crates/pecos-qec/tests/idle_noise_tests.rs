...
fn idle_rates_absent_means_no_idle_contribution() {
    // Config provides no Idle rates and uses zero fallback. DEM should
    // have zero mechanisms: prep/measure are 0 and idle falls back to
    // the per-gate-type default ([p1/3]), which is 0 here.
