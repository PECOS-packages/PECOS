...
fn uniform_equivalent_per_gate_matches_scalar_dem() {
    // DemBuilder with empty-map PerGateTypeNoise (fallback only) should
    ...
    let per_gate = DemBuilder::new(&influence)
        .with_per_gate_noise(PerGateTypeNoise::from_fallback(NoiseConfig::new(0.01, 0.02, 0.005, 0.003)))
        .with_detectors_json(r#"[{"id": 0, "records": [-1]}]"#)
        .unwrap()
        .build();
    ...
    assert_eq!(
        scalar_text.lines().filter(|l| l.starts_with("error(")).count(),
        per_gate_text.lines().filter(|l| l.starts_with("error(")).count(),
...
fn per_gate_override_produces_decomposed_dem_text() {
    ...
    let cfg = PerGateTypeNoise::from_fallback(NoiseConfig::new(0.0, 0.0, 0.0, 0.0))
...
fn per_qubit_cx_override_changes_dem_probabilities() {
    ...
    let cfg_baseline = PerGateTypeNoise::from_fallback(NoiseConfig::new(0.0, 0.0, 0.0, 0.0))
...
fn decomposed_dem_reflects_per_gate_noise() {
    ...
    let cfg = PerGateTypeNoise::from_fallback(NoiseConfig::new(0.005, 0.005, 0.005, 0.005));
    let dem = DemBuilder::new(&influence)
        .with_per_gate_noise(cfg)
        .with_detectors_json(r#"[{"id": 0, "records": [-1]}]"#)
        .unwrap()
