...
fn per_gate_uniform_equivalent_matches_scalar_path() {
    // Build a PerGateTypeNoise that mimics uniform p1=p2=p_meas=p_init=0.01.
    // Expectation: `per_gate.rate_1q()` lookup with empty map falls back
    // to `fallback.p1 / 3.0` for 1Q, and `fallback.p2 / 15.0` for 2Q --
    ...
    let per_gate = DemSamplerBuilder::new(&influence)
        .with_per_gate_noise(PerGateTypeNoise::from_fallback(NoiseConfig::uniform(p)))
        .with_detectors_json(r#"[{"id": 0, "records": [-1]}]"#)
        .unwrap()
        .build();
...
fn per_gate_override_changes_cx_rate() {
    ...
    let per_gate = DemSamplerBuilder::new(&influence)
        .with_per_gate_noise(
            PerGateTypeNoise::from_fallback(NoiseConfig::uniform(0.0))
...
#[test]
fn per_gate_fallback_used_for_unmapped_gate_types() {
    // Specify H explicitly (rates[X, Y, Z]), fall back for CX.
    let dag = build_parity_check_circuit();
    let analyzer = DagFaultAnalyzer::new(&dag);
    let influence = analyzer.build_influence_map();

    let cfg = PerGateTypeNoise::from_fallback(NoiseConfig::uniform(0.01))
