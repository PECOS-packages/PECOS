...
use pecos_qec::fault_tolerance::dem_builder::{
    DemSamplerBuilder, NoiseConfig, PerGateTypeNoise,
};
...
fn per_qubit_measurement_override_takes_precedence() {
    ...
    let mut cfg = PerGateTypeNoise::from_fallback(NoiseConfig::uniform(0.01));
    // Explicitly override q0, leave q1 on the scalar fallback.
...
fn per_qubit_init_override_takes_precedence() {
    let q0 = QubitId::from(0usize);
    let q1 = QubitId::from(1usize);
    let mut cfg = PerGateTypeNoise::from_fallback(NoiseConfig::uniform(0.02));
...
fn per_qubit_measurement_rate_raises_only_targeted_qubit() {
    ...
    let q0 = QubitId::from(0usize);
    let cfg_only_q0 = PerGateTypeNoise::from_fallback(NoiseConfig::new(0.0, 0.0, 0.0, 0.0))
    ...
    let cfg_uniform = PerGateTypeNoise::from_fallback(NoiseConfig::new(0.0, 0.0, 0.05, 0.0));
    ...
    let delta = (sim_only_q0.average_error_probability()
        - sim_uniform.average_error_probability())
    .abs();
    assert!(delta < 1e-12, "per-mech probabilities should match: {}", delta);
}
...
fn per_qubit_init_rate_raises_only_targeted_qubit() {
    ...
    let q1 = QubitId::from(1usize);
    let cfg_only_q1 = PerGateTypeNoise::from_fallback(NoiseConfig::new(0.0, 0.0, 0.0, 0.0))
    ...
    let cfg_uniform = PerGateTypeNoise::from_fallback(NoiseConfig::new(0.0, 0.0, 0.0, 0.05));
    let sim_uniform = DemSamplerBuilder::new(&influence)
        .with_per_gate_noise(cfg_uniform)
        .with_detectors_json(r#"[{"id": 0, "records": [-3]}, {"id": 1, "records": [-2]}, {"id": 2, "records": [-1]}]"#)
        .unwrap()
...
#[test]
fn per_qubit_measurement_path_is_backward_compatible() {
    // A PerGateTypeNoise without any per-qubit measurement rates should
    // fall back to scalar p_meas exactly.
    let dag = build_three_ancilla_circuit();
    let analyzer = DagFaultAnalyzer::new(&dag);
    let influence = analyzer.build_influence_map();

    let cfg = PerGateTypeNoise::from_fallback(NoiseConfig::uniform(0.05));
    ...
    let delta = (sim_per_gate.average_error_probability()
        - sim_scalar.average_error_probability())
    .abs();
