...
//! 2. qubits not in the per-qubit map fall back to per-gate-type.
...
fn per_qubit_override_takes_precedence_over_per_gate_type() {
    ...
    let cfg = PerGateTypeNoise::from_fallback(NoiseConfig::uniform(0.1))
    ...
    // qubit 1 falls back to per-gate-type
    assert_eq!(cfg.rate_1q_on(GateType::H, q1, 0), 0.01);
    assert_eq!(cfg.rate_1q_on(GateType::H, q1, 1), 0.02);

    // Unregistered gate on qubit 0 falls back to per-gate-type (not set),
    // then to uniform fallback.p1/3.
...
fn per_qubit_2q_override_takes_precedence() {
    ...
    let cfg = PerGateTypeNoise::from_fallback(NoiseConfig::uniform(0.0))
    ...
    // (q2, q3) falls back to per-gate-type
...
fn per_qubit_cx_rate_affects_mechanism_probabilities() {
    ...
    //   CX on (1, 4): per-gate-type fallback rate
    ...
    let baseline_cfg = PerGateTypeNoise::from_fallback(NoiseConfig::uniform(0.0))
    ...
    let override_cfg = PerGateTypeNoise::from_fallback(NoiseConfig::uniform(0.0))
        .with_2q_rates(GateType::CX, baseline_rates)
...
#[test]
fn per_qubit_path_is_backward_compatible_with_per_gate_only() {
    // A config with only per-gate-type rates (no per-qubit overrides)
    // should produce identical output to the pre-per-qubit path.
    let dag = build_circuit_with_two_cxs();
    let analyzer = DagFaultAnalyzer::new(&dag);
    let influence = analyzer.build_influence_map();
