...
use pecos_qec::fault_tolerance::dem_builder::{
    DetectorDef, LogicalObservable, NoiseConfig, PerGateTypeNoise,
};
...
fn dem_text_export_with_scalar_noise() {
    ...
    let sim = DemStabSim::builder()
        ...
        .observables(vec![LogicalObservable::new(0).with_records([-1])])
        .build()
        .unwrap();
...
fn dem_text_export_with_per_gate_noise() {
    ...
    let cfg = PerGateTypeNoise::from_fallback(NoiseConfig::new(0.0, 0.0, 0.001, 0.001))
