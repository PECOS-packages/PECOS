    ...
    R: Rng,
{
    ...
    fn fdg(&mut self, qubits: &[QubitId]) -> &mut Self {
        for &q in qubits {
            let q = q.index();
            self.apply_single_qubit(q, |re_a, im_a, re_b, im_b| {
                // FDG = SXDG * SZDG = (1/2)[[1-i, 1-i], [1+i, -1-i]]
                // new_a = (1-i)/2 * a + (1-i)/2 * b
                // new_b = (1+i)/2 * a + (-1-i)/2 * b
                (
                    0.5 * (re_a + im_a + re_b + im_b),
    ...
    fn cx(&mut self, qubits: &[QubitId]) -> &mut Self {
        ...
        for pair in qubits.chunks_exact(2) {
            ...
            // Strided iteration for cache efficiency
        ...
        for pair in qubits.chunks_exact(2) {
            ...
            for i_hi in (0..n).step_by(step_hi * 2) {
                for i_lo in (i_hi..i_hi + step_hi).step_by(step_lo * 2) {
                    for offset in 0..step_lo {
                        let base = i_lo + offset;
                        let idx_01 = base | (1 << q2);
                        let idx_10 = base | (1 << q1);
    ...
    R: Rng,
{
    ...
    fn rz(&mut self, theta: f64, qubits: &[QubitId]) -> &mut Self {
        ...
        for &q in qubits {
            ...
            self.apply_single_qubit(q, |re_a, im_a, re_b, im_b| {
                ...
                (
                    cos_neg * re_a - sin_neg * im_a,
                    sin_neg * re_a + cos_neg * im_a,
                    cos_pos * re_b - sin_pos * im_b,
                    sin_pos * re_b + cos_pos * im_b,
                )
            });
        }
        self
    ...
    fn r1xy(&mut self, theta: f64, phi: f64, qubits: &[QubitId]) -> &mut Self {
        ...
        // r01 = -i*sin*e^(-iφ) = -i*sin*(cosφ - i*sinφ) = -sin*sinφ - i*sin*cosφ
        // r10 = -i*sin*e^(iφ)  = -i*sin*(cosφ + i*sinφ) = sin*sinφ - i*sin*cosφ
        ...
        for &q in qubits {
            ...
            self.apply_single_qubit(q, |re_a, im_a, re_b, im_b| {
                ...
                // new_b = r10*a + cos*b
                (
                    cos * re_a + r01_re * re_b - r01_im * im_b,
                    cos * im_a + r01_re * im_b + r01_im * re_b,
                    r10_re * re_a - r10_im * im_a + cos * re_b,
                    r10_re * im_a + r10_im * re_a + cos * im_b,
                )
            });
        }
        self
