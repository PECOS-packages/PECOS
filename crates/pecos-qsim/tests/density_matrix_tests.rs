...
use pecos_qsim::quantum_simulator::QuantumSimulator;
...
fn test_bell_state_measurement_preserves_correlations() {
    ...
    for seed in [42u64, 123, 456, 789, 1000] {
        ...
        if result.outcome {
            ...
            assert!(dm.probability(0) < 1e-10, "Expected P(00)=0, got {}", dm.probability(0));
            assert!(dm.probability(1) < 1e-10, "Expected P(01)=0, got {}", dm.probability(1));
            assert!(dm.probability(2) < 1e-10, "Expected P(10)=0, got {}", dm.probability(2));
            assert!((dm.probability(3) - 1.0).abs() < 1e-10, "Expected P(11)=1, got {}", dm.probability(3));
        } else {
            // Measured 0 on qubit 0, should be in |00⟩
            assert!((dm.probability(0) - 1.0).abs() < 1e-10, "Expected P(00)=1, got {}", dm.probability(0));
            assert!(dm.probability(1) < 1e-10, "Expected P(01)=0, got {}", dm.probability(1));
            assert!(dm.probability(2) < 1e-10, "Expected P(10)=0, got {}", dm.probability(2));
            assert!(dm.probability(3) < 1e-10, "Expected P(11)=0, got {}", dm.probability(3));
        }

        // Measuring qubit 1 should now be deterministic and match qubit 0
        let result2 = dm.mz(1);
        assert!(result2.is_deterministic);
        assert_eq!(result.outcome, result2.outcome, "Qubit 1 should match qubit 0 in Bell state");
    }
}