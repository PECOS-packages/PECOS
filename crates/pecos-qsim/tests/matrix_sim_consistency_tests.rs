...
fn sim_matches_matrix_1q_cliffords() {
    ...
    for &cliff in Clifford::all_1q() {
        ...
        let matches = if actual[0].norm() < tolerance && expected[0].norm() < tolerance {
            actual.iter().zip(expected.iter()).all(|(a, b)| (a.norm() - b.norm()).abs() < tolerance)
        } else if let Some((a, b)) = actual.iter().zip(expected.iter())
            .find(|(a, b)| a.norm() > tolerance && b.norm() > tolerance) {
            let ratio = b / a;
            actual.iter().zip(expected.iter()).all(|(a, b)| (a * ratio - b).norm() < tolerance)
        } else {
            false
        };
        assert!(matches, "Simulator disagrees with matrix for 1q gate {cliff}");
    }
}
...
fn sim_matches_matrix_2q_cliffords() {
    ...
    let gates: Vec<(Clifford, Box<dyn Fn(&mut StateVec)>)> = vec![
        (Clifford::CX, Box::new(|s: &mut StateVec| { s.cx(&qid2(0, 1)); })),
        (Clifford::CY, Box::new(|s: &mut StateVec| { s.cy(&qid2(0, 1)); })),
        (Clifford::CZ, Box::new(|s: &mut StateVec| { s.cz(&qid2(0, 1)); })),
        (Clifford::SWAP, Box::new(|s: &mut StateVec| { s.swap(&qid2(0, 1)); })),
        (Clifford::SXX, Box::new(|s: &mut StateVec| { s.sxx(&qid2(0, 1)); })),
        (Clifford::SXXdg, Box::new(|s: &mut StateVec| { s.sxxdg(&qid2(0, 1)); })),
        (Clifford::SYY, Box::new(|s: &mut StateVec| { s.syy(&qid2(0, 1)); })),
        (Clifford::SYYdg, Box::new(|s: &mut StateVec| { s.syydg(&qid2(0, 1)); })),
        (Clifford::SZZ, Box::new(|s: &mut StateVec| { s.szz(&qid2(0, 1)); })),
        (Clifford::SZZdg, Box::new(|s: &mut StateVec| { s.szzdg(&qid2(0, 1)); })),
        (Clifford::ISWAP, Box::new(|s: &mut StateVec| { s.iswap(&qid2(0, 1)); })),
        (Clifford::G, Box::new(|s: &mut StateVec| { s.g(&qid2(0, 1)); })),
    ];
    ...
    for (cliff, apply_fn) in &gates {
        ...
        let tolerance = 1e-10;
        let matches = if let Some((a, b)) = actual.iter().zip(expected.iter())
            .find(|(a, b)| a.norm() > tolerance && b.norm() > tolerance) {
            let ratio = b / a;
            actual.iter().zip(expected.iter()).all(|(a, b)| (a * ratio - b).norm() < tolerance)
        } else {
        ...
        assert!(matches, "Simulator disagrees with matrix for 2q gate {cliff}");
    }
}
...
fn sim_matches_matrix_2q_on_superposition() {
    ...
    for (cliff, apply_fn) in &gates {
        ...
        assert!(matches, "Simulator disagrees with matrix on |++> for 2q gate {cliff}");
    }
}
...
fn sxx_squared_is_xx() {
    let input_states: Vec<(&str, Box<dyn Fn(&mut StateVec)>)> = vec![
        ("|00>", Box::new(|_s: &mut StateVec| {})),
        ("|10>", Box::new(|s: &mut StateVec| { s.x(&qid(0)); })),
        ("|01>", Box::new(|s: &mut StateVec| { s.x(&qid(1)); })),
        ("|++>", Box::new(|s: &mut StateVec| { s.h(&qid(0)); s.h(&qid(1)); })),
    ];
...
fn iswapdg_times_iswap_is_identity_matrix() {
    ...
    assert!(diff < 1e-10, "ISWAPdg * ISWAP should be identity, diff = {diff}");

    // Also verify ISWAP * ISWAPdg = I
    let product2 = &iswap_mat * &iswapdg_mat;
    let diff2 = (&product2 - &identity).norm();
    assert!(diff2 < 1e-10, "ISWAP * ISWAPdg should be identity, diff = {diff2}");
}
...
fn g_squared_returns_to_original_state() {
    ...
    for (name, prepare) in &input_states {
        ...
        let identity = UnitaryMatrix::identity(4);
        let diff = (&product - &identity).norm();
        assert!(diff < 1e-10, "Gdg * G should be identity on {name}, diff = {diff}");
    }
}
...
fn gate_then_dagger_1q_identity() {
    ...
    let pairs: Vec<(&str, Box<dyn Fn(&mut StateVec)>, Box<dyn Fn(&mut StateVec)>)> = vec![
        ("SX/SXdg", Box::new(|s: &mut StateVec| { s.sx(&qid(0)); }), Box::new(|s: &mut StateVec| { s.sxdg(&qid(0)); })),
        ("SY/SYdg", Box::new(|s: &mut StateVec| { s.sy(&qid(0)); }), Box::new(|s: &mut StateVec| { s.sydg(&qid(0)); })),
        ("SZ/SZdg", Box::new(|s: &mut StateVec| { s.sz(&qid(0)); }), Box::new(|s: &mut StateVec| { s.szdg(&qid(0)); })),
        ("F/Fdg", Box::new(|s: &mut StateVec| { s.f(&qid(0)); }), Box::new(|s: &mut StateVec| { s.fdg(&qid(0)); })),
        ("F2/F2dg", Box::new(|s: &mut StateVec| { s.f2(&qid(0)); }), Box::new(|s: &mut StateVec| { s.f2dg(&qid(0)); })),
        ("F3/F3dg", Box::new(|s: &mut StateVec| { s.f3(&qid(0)); }), Box::new(|s: &mut StateVec| { s.f3dg(&qid(0)); })),
        ("F4/F4dg", Box::new(|s: &mut StateVec| { s.f4(&qid(0)); }), Box::new(|s: &mut StateVec| { s.f4dg(&qid(0)); })),
    ];

    // Self-inverse 1q gates (H variants are all self-inverse)
    let self_inverse: Vec<(&str, Box<dyn Fn(&mut StateVec)>)> = vec![
        ("H", Box::new(|s: &mut StateVec| { s.h(&qid(0)); })),
        ("X", Box::new(|s: &mut StateVec| { s.x(&qid(0)); })),
        ("Y", Box::new(|s: &mut StateVec| { s.y(&qid(0)); })),
        ("Z", Box::new(|s: &mut StateVec| { s.z(&qid(0)); })),
        ("H2", Box::new(|s: &mut StateVec| { s.h2(&qid(0)); })),
        ("H3", Box::new(|s: &mut StateVec| { s.h3(&qid(0)); })),
        ("H4", Box::new(|s: &mut StateVec| { s.h4(&qid(0)); })),
        ("H5", Box::new(|s: &mut StateVec| { s.h5(&qid(0)); })),
        ("H6", Box::new(|s: &mut StateVec| { s.h6(&qid(0)); })),
    ];

    let input_states: Vec<(&str, Box<dyn Fn(&mut StateVec)>)> = vec![
        ("|0>", Box::new(|_s: &mut StateVec| {})),
        ("|1>", Box::new(|s: &mut StateVec| { s.x(&qid(0)); })),
        ("|+>", Box::new(|s: &mut StateVec| { s.h(&qid(0)); })),
    ];
...
fn gate_then_dagger_2q_identity() {
    ...
    let pairs: Vec<(&str, Box<dyn Fn(&mut StateVec)>, Box<dyn Fn(&mut StateVec)>)> = vec![
        ("SXX/SXXdg", Box::new(|s: &mut StateVec| { s.sxx(&qid2(0, 1)); }), Box::new(|s: &mut StateVec| { s.sxxdg(&qid2(0, 1)); })),
        ("SYY/SYYdg", Box::new(|s: &mut StateVec| { s.syy(&qid2(0, 1)); }), Box::new(|s: &mut StateVec| { s.syydg(&qid2(0, 1)); })),
        ("SZZ/SZZdg", Box::new(|s: &mut StateVec| { s.szz(&qid2(0, 1)); }), Box::new(|s: &mut StateVec| { s.szzdg(&qid2(0, 1)); })),
        ("ISWAP/ISWAPdg", Box::new(|s: &mut StateVec| { s.iswap(&qid2(0, 1)); }), Box::new(|s: &mut StateVec| { s.iswapdg(&qid2(0, 1)); })),
    ];

    let self_inverse: Vec<(&str, Box<dyn Fn(&mut StateVec)>)> = vec![
        ("CX", Box::new(|s: &mut StateVec| { s.cx(&qid2(0, 1)); })),
        ("CY", Box::new(|s: &mut StateVec| { s.cy(&qid2(0, 1)); })),
        ("CZ", Box::new(|s: &mut StateVec| { s.cz(&qid2(0, 1)); })),
        ("SWAP", Box::new(|s: &mut StateVec| { s.swap(&qid2(0, 1)); })),
        ("G", Box::new(|s: &mut StateVec| { s.g(&qid2(0, 1)); })),
    ];
...
fn sim_matches_matrix_2q_nonadjacent_on_zero_state() {
    ...
    let gates: Vec<(Clifford, Box<dyn Fn(&mut StateVec)>)> = vec![
        (Clifford::CX, Box::new(|s: &mut StateVec| { s.cx(&qid2(0, 2)); })),
        (Clifford::CY, Box::new(|s: &mut StateVec| { s.cy(&qid2(0, 2)); })),
        (Clifford::CZ, Box::new(|s: &mut StateVec| { s.cz(&qid2(0, 2)); })),
        (Clifford::SWAP, Box::new(|s: &mut StateVec| { s.swap(&qid2(0, 2)); })),
        (Clifford::SXX, Box::new(|s: &mut StateVec| { s.sxx(&qid2(0, 2)); })),
        (Clifford::SXXdg, Box::new(|s: &mut StateVec| { s.sxxdg(&qid2(0, 2)); })),
        (Clifford::SYY, Box::new(|s: &mut StateVec| { s.syy(&qid2(0, 2)); })),
        (Clifford::SYYdg, Box::new(|s: &mut StateVec| { s.syydg(&qid2(0, 2)); })),
        (Clifford::SZZ, Box::new(|s: &mut StateVec| { s.szz(&qid2(0, 2)); })),
        (Clifford::SZZdg, Box::new(|s: &mut StateVec| { s.szzdg(&qid2(0, 2)); })),
        (Clifford::ISWAP, Box::new(|s: &mut StateVec| { s.iswap(&qid2(0, 2)); })),
        (Clifford::ISWAPdg, Box::new(|s: &mut StateVec| { s.iswapdg(&qid2(0, 2)); })),
        (Clifford::G, Box::new(|s: &mut StateVec| { s.g(&qid2(0, 2)); })),
        (Clifford::Gdg, Box::new(|s: &mut StateVec| { s.gdg(&qid2(0, 2)); })),
    ];

    for (cliff, apply_fn) in &gates {
        let ur = cliff.to_unitary_rep_on_qubits(0usize, 2usize);
        let mat = to_matrix(&ur);
        assert_eq!(mat.nrows(), 8, "Non-adjacent gate should produce 8x8 matrix");
...
fn gate_then_dagger_2q_nonadjacent_identity() {
    ...
    let pairs: Vec<(&str, Box<dyn Fn(&mut StateVec)>, Box<dyn Fn(&mut StateVec)>)> = vec![
        ("SXX/SXXdg", Box::new(|s: &mut StateVec| { s.sxx(&qid2(0, 2)); }), Box::new(|s: &mut StateVec| { s.sxxdg(&qid2(0, 2)); })),
        ("SYY/SYYdg", Box::new(|s: &mut StateVec| { s.syy(&qid2(0, 2)); }), Box::new(|s: &mut StateVec| { s.syydg(&qid2(0, 2)); })),
        ("SZZ/SZZdg", Box::new(|s: &mut StateVec| { s.szz(&qid2(0, 2)); }), Box::new(|s: &mut StateVec| { s.szzdg(&qid2(0, 2)); })),
        ("ISWAP/ISWAPdg", Box::new(|s: &mut StateVec| { s.iswap(&qid2(0, 2)); }), Box::new(|s: &mut StateVec| { s.iswapdg(&qid2(0, 2)); })),
    ];

    let self_inverse: Vec<(&str, Box<dyn Fn(&mut StateVec)>)> = vec![
        ("CX", Box::new(|s: &mut StateVec| { s.cx(&qid2(0, 2)); })),
        ("CZ", Box::new(|s: &mut StateVec| { s.cz(&qid2(0, 2)); })),
        ("SWAP", Box::new(|s: &mut StateVec| { s.swap(&qid2(0, 2)); })),
        ("G", Box::new(|s: &mut StateVec| { s.g(&qid2(0, 2)); })),
    ];

    let input_states: Vec<(&str, Box<dyn Fn(&mut StateVec)>)> = vec![
        ("|000>", Box::new(|_s: &mut StateVec| {})),
        ("|100>", Box::new(|s: &mut StateVec| { s.x(&qid(0)); })),
        ("|001>", Box::new(|s: &mut StateVec| { s.x(&qid(2)); })),
        ("H(0)H(2)|000>", Box::new(|s: &mut StateVec| { s.h(&qid(0)); s.h(&qid(2)); })),
        ("Bell(0,1)", Box::new(|s: &mut StateVec| { s.h(&qid(0)); s.cx(&qid2(0, 1)); })),
    ];
