...
fn test_builtin_prepare_plus_state_difference() {
    ...
    for i in 0..(1 << num_qubits) {
        ...
        assert!((p1 - 0.0625).abs() < 1e-10, "StateVec direct: {} should be 0.0625", p1);
        ...
        assert!((p2 - 0.25).abs() < 1e-10, "StateVec with H: {} should be 0.25", p2);
        assert!((p3 - 0.25).abs() < 1e-10, "DensityMatrix: {} should be 0.25", p3);
    }
