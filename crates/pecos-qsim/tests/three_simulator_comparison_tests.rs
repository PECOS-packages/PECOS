...
fn assert_probs_equal(p1: f64, p2: f64) {
    assert!((p1 - p2).abs() < 1e-10, "Probabilities differ: {} vs {}", p1, p2);
}

// Helper function to compare probabilities for all three simulators
fn compare_all_probabilities(sv: &StateVec, dm: &DensityMatrix, stab: &StdSparseStab, num_qubits: usize) {
...
where F: Fn(&mut StateVec, &mut DensityMatrix, &mut StdSparseStab)
{
...
where F: Fn(&mut StateVec, &mut DensityMatrix)
{
    let seed = 42; // Fixed seed for determinism
    let mut sv = StateVec::with_seed(num_qubits, seed);
    let mut dm = DensityMatrix::with_seed(num_qubits, seed);
