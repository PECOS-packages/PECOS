...
use pecos_quest::{QuestStateVec, QuantumSimulator, CliffordGateable, MeasurementResult};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for measurement_round in 1..=5 {
        ...
        println!("  Round {}: Qubit 0: {} | Qubit 1: {} | Correlated: {}", 
                 measurement_round,
                 if result0.outcome { "1" } else { "0" },
                 if result1.outcome { "1" } else { "0" },
                 if result0.outcome == result1.outcome { "✓" } else { "✗" });
    }
    println!();
...
fn display_state_probabilities(state: &QuestStateVec) {
    ...
    for i in 0..num_states {
        ...
        if prob > 1e-10 {  // Only show non-zero probabilities
            println!("  |{:0width$b}⟩: {:.6}", i, prob, width = state.num_qubits());
        }
    }
}