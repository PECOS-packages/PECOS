...
#[cfg(test)]
mod tests {
    use crate::{QuestStateVec, QuestDensityMatrix};
    use pecos_qsim::{QuantumSimulator, CliffordGateable, ArbitraryRotationGateable, MeasurementResult};
    ...
    fn test_hadamard_gate() {
        ...
        assert_complex_eq(sim.get_amplitude(0), Complex64::new(sqrt2_inv, 0.0), EPSILON);
        assert_complex_eq(sim.get_amplitude(1), Complex64::new(sqrt2_inv, 0.0), EPSILON);
    }
    ...
    fn test_bell_state_preparation() {
        ...
        let sqrt2_inv = 1.0 / 2.0_f64.sqrt();
        assert_complex_eq(sim.get_amplitude(0b00), Complex64::new(sqrt2_inv, 0.0), EPSILON);
        ...
        assert_complex_eq(sim.get_amplitude(0b11), Complex64::new(sqrt2_inv, 0.0), EPSILON);
    }
    ...
    fn test_measurement() {
        ...
        let result = sim.mz(0);
        assert_eq!(result.outcome, false); // 0 outcome
        assert_eq!(result.is_deterministic, true);
    ...
    fn test_measurement_after_x() {
        ...
        let result = sim.mz(0);
        assert_eq!(result.outcome, true); // 1 outcome
        assert_eq!(result.is_deterministic, true);
    }
