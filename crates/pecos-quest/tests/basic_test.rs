...
fn test_state_with_seed() {
    ...
    assert_relative_eq!(state1.probability(0), state2.probability(0), epsilon = 1e-10);
}
...
fn test_measurement() {
    ...
    // We can't assert the exact outcome since it's random
    assert!(result.outcome || !result.outcome); // Just checking the result is valid
}
