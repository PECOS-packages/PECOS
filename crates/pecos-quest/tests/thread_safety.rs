...
use pecos_quest::{QuestStateVec, QuantumSimulator, CliffordGateable, ArbitraryRotationGateable};
use std::thread;
...
fn test_parallel_independent_instances() {
    ...
    let handles: Vec<_> = (0..NUM_THREADS)
        .map(|thread_id| {
            ...
            thread::spawn(move || {
                ...
                let mut state: QuestStateVec<ChaCha8Rng> = QuestStateVec::with_seed(NUM_QUBITS, thread_id as u64 + 42);
    ...
    let results: Vec<f64> = handles.into_iter()
...
fn test_parallel_bell_state_measurements() {
    ...
    let handles: Vec<_> = (0..NUM_THREADS)
        .map(|thread_id| {
            thread::spawn(move || {
                let mut state: QuestStateVec<ChaCha8Rng> = QuestStateVec::with_seed(2, thread_id as u64 * 1000);
    ...
    for (thread_id, correlation_count, total_measurements) in results {
        println!("Thread {}: {}/{} correlated measurements", 
                 thread_id, correlation_count, total_measurements);
        
        // Bell state measurements should be perfectly correlated
        // (allowing for potential QuEST measurement implementation details)
        assert_eq!(correlation_count, total_measurements, 
                  "Thread {} had imperfect Bell state correlation", thread_id);
    }
}
...
fn test_parallel_cloning_and_states() {
    ...
    let handles: Vec<_> = (0..NUM_THREADS)
        .map(|thread_id| {
            thread::spawn(move || {
                ...
                let mut template: QuestStateVec<ChaCha8Rng> = QuestStateVec::with_seed(2, 12345); // Same seed
    ...
    for (thread_id, probs) in &results {
        println!("Thread {}: probabilities = {:?}", thread_id, probs);
...
fn test_many_parallel_instances() {
    ...
    let handles: Vec<_> = (0..NUM_THREADS)
        .map(|thread_id| {
            thread::spawn(move || {
                let mut state: QuestStateVec<ChaCha8Rng> = QuestStateVec::with_seed(1, thread_id as u64);
    ...
    println!("All {} threads completed successfully", NUM_THREADS);
    for (thread_id, outcome) in results {
        println!("Thread {}: final measurement = {}", 
                 thread_id, if outcome { "1" } else { "0" });
    }
}