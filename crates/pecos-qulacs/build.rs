use pecos_build_utils::{download_cached, extract_archive, qulacs_download_info, eigen_download_info, boost_download_info};
use std::env;
...
fn main() {
    ...
    // Extract archives
    let qulacs_path = extract_archive(&qulacs_data, &out_dir, Some("qulacs"))
        .expect("Failed to extract Qulacs");
    let eigen_path = extract_archive(&eigen_data, &out_dir, Some("eigen"))
        .expect("Failed to extract Eigen");
    let boost_path = extract_archive(&boost_data, &out_dir, Some("boost"))
        .expect("Failed to extract Boost");
