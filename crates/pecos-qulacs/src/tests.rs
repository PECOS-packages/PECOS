...
mod tests {
    ...
    fn assert_states_equal(state1: &[Complex64], state2: &[Complex64], tolerance: f64) {
        assert_eq!(state1.len(), state2.len(), "State vectors have different lengths");
        for (i, (a, b)) in state1.iter().zip(state2.iter()).enumerate() {
            let diff = (a - b).norm();
            assert!(
                diff < tolerance,
                "States differ at index {}: |{:?} - {:?}| = {} >= {}",
                i, a, b, diff, tolerance
            );
        }
    }
    ...
    fn test_two_qubit_gates() {
        ...
        assert!(found_one_state, "SWAP gate should result in exactly one basis state");
    }
    ...
    fn test_seed_determinism() {
        ...
        assert_eq!(results1, results2, "Same seed should produce same measurement results");
    }
    ...
    fn test_different_seeds_give_different_results() {
        ...
        assert_ne!(results1, results2, "Different seeds should produce different results");
    }
    ...
    fn test_state_normalization() {
        ...
        assert!((norm_squared - 1.0).abs() < 1e-10, "State should remain normalized");
    }
    ...
    fn test_qubit_ordering() {
        ...
        assert_eq!(nonzero_idx, 8, "X on qubit 0 should produce state |1000> (index 8)");
        ...
        assert_eq!(nonzero_idx, 2, "X on qubit 2 should produce state |0010> (index 2)");
    }
    ...
    fn test_measurement_statistics() {
        ...
        let ratio = count_zero as f64 / n_trials as f64;
        assert!((ratio - 0.5).abs() < 0.05, "Measurement statistics should be ~50/50 for |+⟩ state");
    }
    ...
    fn test_measurement_collapse() {
        ...
        if result.outcome {
            ...
            assert!(final_vector[0].norm() < 1e-10, "After measuring |1⟩, amplitude of |0⟩ should be 0");
            assert!((final_vector[1] - Complex64::new(1.0, 0.0)).norm() < 1e-10, "After measuring |1⟩, amplitude of |1⟩ should be 1");
        } else {
            // Should collapse to |0⟩
            assert!((final_vector[0] - Complex64::new(1.0, 0.0)).norm() < 1e-10, "After measuring |0⟩, amplitude of |0⟩ should be 1");
            assert!(final_vector[1].norm() < 1e-10, "After measuring |0⟩, amplitude of |1⟩ should be 0");
        }
    }
}