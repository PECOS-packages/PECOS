...
mod thread_safety_tests {
    ...
    fn test_clone_and_thread_independence() {
        ...
        let handle1 = thread::spawn(move || {
            ...
            let state = sim.state();
            results1.lock().unwrap().push(("thread1", state[0], state[3]));
        });
        ...
        let handle2 = thread::spawn(move || {
            ...
            sim.h(1usize);
            let state = sim.state();
            results2.lock().unwrap().push(("thread2", state[1], state[3]));
        });
        ...
        let handle3 = thread::spawn(move || {
            ...
            sim.h(1usize);
            let state = sim.state();
            results3.lock().unwrap().push(("thread3", state[0], state[3]));
        });
    ...
    fn test_concurrent_monte_carlo_simulation() {
        ...
        let handles: Vec<_> = (0..NUM_THREADS)
            .map(|thread_id| {
                ...
                sim.set_rng(ChaCha8Rng::seed_from_u64(123 + thread_id as u64 * 1000)).unwrap();
        ...
        for handle in handles {
            ...
            println!("Thread {}: {} ones out of {} trials ({:.1}%)", 
                    thread_id, ones_count, trials, 
        ...
        println!("Overall: {} ones out of {} trials ({:.1}%)", 
                total_ones, total_trials, overall_ratio * 100.0);
        
        // Should be approximately 50% (allowing some variance)
        assert!((overall_ratio - 0.5).abs() < 0.1, 
                "Expected ~50% measurement outcomes, got {:.1}%", overall_ratio * 100.0);
    }
}