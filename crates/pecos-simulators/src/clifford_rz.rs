...
#[derive(Clone, Debug)]
pub struct CliffordRzGeneric<S: IndexSet = BitSet, R: SeedableRng + Rng + Debug = PecosRng> {
    num_qubits: usize,
...
pub type CliffordRz<R = PecosRng> = CliffordRzGeneric<BitSet, R>;

/// Builder for configuring a `CliffordRz` simulator.
pub struct CliffordRzBuilder {
    num_qubits: usize,
    seed: Option<u64>,
    rel_pruning_threshold: f64,
    mc_threshold: Option<usize>,
...
impl<S: IndexSet, R: SeedableRng + Rng + Debug + Clone> CliffordRzGeneric<S, R> {
...
impl CliffordRzGeneric<BitSet, PecosRng> {
    /// Create a builder for configuring the simulator.
    #[must_use]
    pub fn builder(num_qubits: usize) -> CliffordRzBuilder {
        CliffordRzBuilder {
            num_qubits,
            seed: None,
            rel_pruning_threshold: 1e-8,
...
impl<S: IndexSet, R: SeedableRng + Rng + Debug + Clone> QuantumSimulator
    for CliffordRzGeneric<S, R>
{
...
impl<S: IndexSet, R: SeedableRng + Rng + Debug + Clone> CliffordGateable
    for CliffordRzGeneric<S, R>
{
...
mod tests {
    ...
    #[test]
    fn test_clifford_rz_initial_state() {
        let mut sim = CliffordRz::new(2);
    ...
    #[test]
    fn test_mixed_clifford_rz_circuit() {
        let mut crz = CliffordRz::new(2);
        let mut sv = StateVec::new(2);
    ...
    fn test_measurement_deterministic_zero_state() {
        let mut crz = CliffordRz::new_with_seed(1, 42);
    ...
    fn test_measurement_statistics() {
        ...
        for seed in 0..num_shots {
            let mut crz = CliffordRz::new_with_seed(1, seed);
            crz.rx(theta, &qid(0));
            let results = crz.mz(&qid(0));
    ...
    fn test_measurement_bell_state_with_rz() {
        ...
        let theta = Angle64::from_radians(0.6);
        let mut crz = CliffordRz::new_with_seed(2, 42);
        crz.h(&qid(0))
            .cx(&[(QubitId(0), QubitId(1))])
            .rz(theta, &qid(0));
    ...
    fn test_measurement_probability_matches_statevec() {
        // Compare exact measurement probabilities between CliffordRz and StateVec.
    ...
    fn test_measurement_statistics_2qubit() {
        ...
        // Sample from CliffordRz
        let mut counts = [0u32; 4];
        for seed in 0..num_shots {
            let mut crz = CliffordRz::new_with_seed(2, seed);
            crz.h(&qid(0))
                .rz(theta, &qid(0))
                .cx(&[(QubitId(0), QubitId(1))]);
            let r0 = crz.mz(&qid(0));
    ...
    fn test_rzz_then_measurement() {
        ...
        // Verify CliffordRz state matches before measurement
        let mut crz = CliffordRz::new(2);
        crz.h(&qid(0))
            .h(&qid(1))
            .rzz(theta, &[(QubitId(0), QubitId(1))]);
    ...
    fn test_5_qubit_circuit() {
        // Verify CliffordRz works at 5 qubits with entanglement and RZ gates.
        let mut crz = CliffordRz::new(5);
    ...
    fn test_5_qubit_measurement() {
        ...
        let mut crz = CliffordRz::new_with_seed(5, 42);
        crz.h(&[QubitId(0)])
            .cx(&[(QubitId(0), QubitId(1))])
            .cx(&[(QubitId(1), QubitId(2))])
            .rz(theta, &[QubitId(0)])
    ...
    fn test_builder_default() {
        let mut sim = CliffordRz::builder(2).build();
    ...
    fn test_builder_with_seed() {
        let mut sim1 = CliffordRz::builder(1).seed(42).build();
        let mut sim2 = CliffordRz::builder(1).seed(42).build();
        sim1.h(&qid(0));
        sim2.h(&qid(0));
    ...
    fn test_builder_exact_mode() {
        ...
        let mut sim = CliffordRz::builder(1)
    ...
    fn test_builder_aggressive_pruning() {
        ...
        let mut sim = CliffordRz::builder(4)
    ...
    fn test_exact_mode_matches_statevec() {
        ...
        let mut crz = CliffordRz::builder(2)
            .pruning_threshold(0.0)
            .seed(42)
            .build();
        let mut sv = StateVec::new(2);
    ...
    /// Test `CliffordRz` at qubit counts that exercise the pairwise inner product
    /// measurement path (n>6) and various `ExponentialSum` tiers.
    #[test]
    fn test_clifford_rz_medium_qubit_counts() {
        // These exercise: n>6 pairwise measurement, ExponentialSum d>3 path
        for nq in [8, 10, 14, 20] {
            let mut crz = CliffordRz::new_with_seed(nq, 42);
    ...
    fn test_high_depth_measurement_matches_statevec() {
        ...
        for nrz in [2, 3, 4] {
            ...
            assert!(
                ...
                "nrz={nrz}: Pr(q0=0) CliffordRz={crz_p0:.3} vs StateVec={sv_p0:.3}"
            );
        }
    }
    ...
    #[test]
    fn test_clifford_rz_measurement_at_pairwise_threshold() {
    ...
    #[test]
    fn test_clifford_rz_at_u64_boundary() {
    ...
    #[test]
    fn test_clifford_rz_at_u128_boundary() {
