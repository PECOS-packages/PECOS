...
soft-output pipeline. Under the integer `maxlog_int` metric, each terminal mass
is instead the best-route (Viterbi) mass for that logical class, the gap is a
route-mass margin, and `log_evidence` is the winning route mass rather than
evidence.
