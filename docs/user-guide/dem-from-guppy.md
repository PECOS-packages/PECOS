...
- `idle_after_2q_duration=<float>` inserts an `Idle` of that duration on both
  qubits after every two-qubit gate.
...
`idle_after_2q_duration`. Idle rates are per time unit: for example, uniform
idle noise uses `p_idle * duration`, clamped to the probability range. T1 and
T2 values must use the same units as the idle duration.
...
# Construct a decoder directly when you need per-shot results.
syndrome = batch.get_syndrome(0)
tesseract = TesseractDecoder.from_dem(dem.to_string(), preset="default")
