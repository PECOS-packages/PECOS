...
If the base letters do not form a Pauli string (one letter per qubit), append the qubit count as `<n>Q`. Thus `RXX` stays unchanged because `XX` is a two-qubit Pauli string, while `RXY1Q` names a rotation in the single-qubit XY plane. A two-qubit rotation generated by `X ⊗ Y` would be `RXY2Q`. The older names `U2q` (two-qubit unitary) and the hardware alias `U1q` predate this rule and keep their lowercase `q`.
