...
cargo run -p pecos-deps -- llvminstall
...
=== "macOS"
    ...
    cargo run -p pecos-deps -- llvmconfigure
    cargo build --features llvm
    ```
...
cargo run -p pecos-deps -- llvmcheck

# Show LLVM version and path
cargo run -p pecos-deps -- llvmversion

# Find LLVM installation path
cargo run -p pecos-deps -- llvmfind
```

## pecos-llvm CLI Reference

The `pecos-deps llvm` CLI tool provides several useful commands:
...
cargo run -p pecos-deps -- llvminstall --force

# Skip automatic configuration after install
cargo run -p pecos-deps -- llvminstall --no-configure
```
...
cargo run -p pecos-deps -- llvmcheck --quiet
```
...
cargo run -p pecos-deps -- llvmfind --export
```
...
```bash
cargo run -p pecos-deps -- llvmvalidate /path/to/llvm
```
...
```bash
cargo run -p pecos-deps -- llvmtool llvm-as
cargo run -p pecos-deps -- llvmtool clang
cargo run -p pecos-deps -- llvmtool llvm-link
```
