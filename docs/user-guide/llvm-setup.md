...
=== "macOS"
    ...
    cargo run -p pecos-dev -- llvm configure
    cargo build --features llvm
    ```
...
cargo run -p pecos-dev -- llvm check

# Show LLVM version and path
cargo run -p pecos-dev -- llvm version

# Find LLVM installation path
cargo run -p pecos-dev -- llvm find
```
...
cargo run -p pecos-dev -- llvm install --force

# Skip automatic configuration after install
cargo run -p pecos-dev -- llvm install --no-configure
```
...
cargo run -p pecos-dev -- llvm check --quiet
```
...
cargo run -p pecos-dev -- llvm find --export
```
...
```bash
cargo run -p pecos-dev -- llvm validate /path/to/llvm
```
...
```bash
cargo run -p pecos-dev -- llvm tool llvm-as
cargo run -p pecos-dev -- llvm tool clang
cargo run -p pecos-dev -- llvm tool llvm-link
```
