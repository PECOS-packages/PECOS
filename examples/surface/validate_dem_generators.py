...
NOISE_CONFIGS = [
    ...
    ("p_meas only", dict(p_meas=0.01), "stabilizer"),
    ("p_prep only", dict(p_prep=0.01), "stabilizer"),
    ("p1 only", dict(p1=0.01), "stabilizer"),
    ("p2 only", dict(p2=0.01), "stabilizer"),
    ("depol all", dict(p1=0.005, p2=0.005, p_meas=0.005, p_prep=0.005), "stabilizer"),
    ("depol strong", dict(p1=0.01, p2=0.01, p_meas=0.01, p_prep=0.01), "stabilizer"),
]

# Coherent noise configs (ground truth: statevec, small circuits only)
COHERENT_CONFIGS = [
    ("idle_rz only", dict(idle_rz=0.05), "statevec"),
    ("rz+depol", dict(idle_rz=0.05, p1=0.005, p2=0.005, p_meas=0.005, p_prep=0.005), "statevec"),
]
...
def ground_truth_rates(tc, noise_kw, shots, seed, det_json, num_meas, num_dets, simulator="stabilizer"):
    ...
    from pecos_rslib_exp import sim_neo, stabilizer, statevec, depolarizing
...
def max_rel_error(test_rates, ref_rates, min_rate=0.003):
    ...
    max_err = 0.0
    for t, r in zip(test_rates, ref_rates):
        if r > min_rate:
    ...
    for distance in distances:
        for basis in bases:
          for circuit_source in circuit_sources:
            try:
                tc = build_circuit(distance, distance, basis, circuit_source, fill_idle)
            ...
            for noise_label, noise_kw, gt_simulator in all_configs:
                ...
                try:
                    ref = ground_truth_rates(
                        tc, noise_kw, shots, seed, det_json, num_meas, num_dets,
...
def main():
    ...
    parser.add_argument(
        "--distance", "-d", type=int, nargs="+", default=[2, 3],
        help="Code distances to test (default: 2 3)"
    )
    parser.add_argument(
        "--basis", choices=["X", "Z"], nargs="+", default=["Z", "X"],
    ...
    parser.add_argument(
        "--circuit-source", choices=["abstract", "traced_qis", "both"],
        default="abstract",
        help="Circuit construction pipeline (default: abstract)"
    )
    parser.add_argument(
        "--fill-idle", action="store_true",
        help="Insert Idle(1) gates on inactive qubits (needed for idle_rz noise)"
    )
    args = parser.parse_args()

    if args.circuit_source == "both":
        sources = ["abstract", "traced_qis"]
    else:
        sources = [args.circuit_source]
