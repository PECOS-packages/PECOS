...
| `exhausted_message_always_plural` | In `exp/pecos-bp-trellis/src/lib.rs`, replace `if self.rungs_tried == 1 { "" } else { "s" }` with `"s"`. | `exhausted_ladder_reports_the_attempted_rung_count` | `assertion &#96;left == right&#96; failed` |
