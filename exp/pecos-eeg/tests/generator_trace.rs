...
fn build_d2_zbasis() -> Vec<Gate> {
    vec![
        ...
        gate(GateType::CX, &[1, 6]), gate(GateType::CX, &[5, 3]),  // tick 3
        gate(GateType::CX, &[3, 6]), gate(GateType::CX, &[5, 2]),  // tick 4
        gate(GateType::CX, &[4, 1]), gate(GateType::CX, &[0, 6]),  // tick 5
        gate(GateType::CX, &[4, 0]), gate(GateType::CX, &[2, 6]),  // tick 6
        gate(GateType::H, &[4]), gate(GateType::H, &[5]),
        ...
        // Round 2
        gate(GateType::H, &[4]), gate(GateType::H, &[5]),
        gate(GateType::CX, &[1, 6]), gate(GateType::CX, &[5, 3]),  // tick 11
        gate(GateType::CX, &[3, 6]), gate(GateType::CX, &[5, 2]),  // tick 12
        gate(GateType::CX, &[4, 1]), gate(GateType::CX, &[0, 6]),  // tick 13
        gate(GateType::CX, &[4, 0]), gate(GateType::CX, &[2, 6]),  // tick 14
        gate(GateType::H, &[4]), gate(GateType::H, &[5]),
        gate(GateType::MZ, &[4]), gate(GateType::MZ, &[5]),
        gate(GateType::MZ, &[6]),
        // Final data readout
...
fn trace_d2_zbasis_generators() {
    ...
    eprintln!("Expanded: {} qubits ({} orig + {} aux), {} measurements",
    ...
    let _dets = vec![
        Detector { id: 1, stabilizer: d1_stab.clone() },
        Detector { id: 2, stabilizer: d2_stab.clone() },
    ];
    ...
    for g in &h_gens {
        ...
        if flips_d1 || flips_d2 {
            ...
            eprintln!("  {:?} coeff={:.6} -> orig={:?} flips: D1={} D2={}",
                g.label, g.coeff, orig, flips_d1, flips_d2);

            if flips_d1 { d1_gens.push((g.label.clone(), g.coeff)); }
            if flips_d2 { d2_gens.push((g.label.clone(), g.coeff)); }
        }
    }
    ...
    for (l, c) in &d1_gens { *d1_bch.entry(l.clone()).or_default() += c; }
    for (l, c) in &d2_gens { *d2_bch.entry(l.clone()).or_default() += c; }

    eprintln!("\nD1 after BCH: {} distinct labels", d1_bch.len());
    for (l, c) in &d1_bch { eprintln!("  {:?} rate={:.6}", l, c); }

    eprintln!("\nD2 after BCH: {} distinct labels", d2_bch.len());
    for (l, c) in &d2_bch { eprintln!("  {:?} rate={:.6}", l, c); }

    // Verify asymmetry in generator counts
    assert_ne!(d1_bch.len(), d2_bch.len(),
    ...
    // Dump raw SparseStab generators
    {
        ...
        for g in &gates_pre {
            ...
            if qs.is_empty() { continue; }
            match g.gate_type {
                GateType::PZ => { for &q in &qs { sim2.pz(&[q]); } }
                GateType::H => { sim2.h(&qs); }
    ...
    let z_all = Bm::z(0).multiply(&Bm::z(1)).multiply(&Bm::z(2)).multiply(&Bm::z(3));
    ...
    eprintln!("Z0Z1Z2: {:?}", stab_group.is_stabilizer(&Bm::z(0).multiply(&Bm::z(1)).multiply(&Bm::z(2))));
    eprintln!("Z1Z2: {:?}", stab_group.is_stabilizer(&Bm::z(1).multiply(&Bm::z(2))));
    ...
    for (det_name, bch) in [("D1", &d1_bch), ("D2", &d2_bch)] {
        ...
        eprintln!("  Off-diagonal: {offdiag:.8} (+{offdiag_plus} pairs, -{offdiag_minus} pairs, 0:{offdiag_zero} pairs, anticommute:{offdiag_anticommute})");
...
fn exclude_final_readout(gates: &[Gate]) -> Vec<Gate> {
    ...
    for g in gates {
        if past_init && (g.gate_type == GateType::PZ || g.gate_type == GateType::QAlloc) {
            for q in &g.qubits { ancilla_qubits.insert(q.index()); }
        }
        if g.gate_type != GateType::PZ && g.gate_type != GateType::QAlloc { past_init = true; }
    }
    let mut end = gates.len();
    for g in gates.iter().rev() {
        if g.gate_type != GateType::MZ { break; }
        if g.qubits.iter().all(|q| !ancilla_qubits.contains(&q.index())) {
            end -= 1;
        } else { break; }
    }
    gates[..end].to_vec()
