...
fn test_h_correction_matches_exact() {
    ...
    for &h in &[0.01, 0.05, 0.1, 0.2] {
        ...
        assert!((p1.total - h * h).abs() < 1e-10,
            "h={h}: EEG p(1)={:.6} expected h²={:.6}", p1.total, h * h);

        // Check closeness to exact
        let rel_err = (p1.total - exact).abs() / exact;
        eprintln!("h={h:.2}: EEG={:.6} exact={exact:.6} rel_err={rel_err:.4}", p1.total);
        if h <= 0.1 {
...
fn test_bell_h_z_invisible() {
    ...
    for outcome in &[vec![false, false], vec![true, true], vec![false, true], vec![true, false]] {
        let p = outcome_probability(&gens, outcome, &stabs);
        assert!(p.h_correction.abs() < 1e-10,
            "H_Z should be invisible: outcome={outcome:?} h_corr={}", p.h_correction);
    }
}
...
fn test_bell_h_x_shifts() {
    ...
    eprintln!("Bell + H_X0: p00={:.6} p11={:.6} p01={:.6} p10={:.6}",
    ...
    assert!((sum - 1.0).abs() < 0.01,
        "Conservation: sum={sum:.6}");

    // Symmetry: p(01) = p(10) (X0 on symmetric Bell state)
    assert!((p01.total - p10.total).abs() < 1e-10,
        "Symmetry: p01={:.6} p10={:.6}", p01.total, p10.total);
}
...
fn test_two_h_generators_interference() {
    ...
    let gens = vec![
        PropagatedEeg { eeg_type: EegType::H, label: Bm::x(0), label2: None, coeff: h1, source: None },
        PropagatedEeg { eeg_type: EegType::H, label: Bm::y(0), label2: None, coeff: h2, source: None },
    ];
    ...
    eprintln!("Two H gens: p0={:.6} p1={:.6} diagonal={diagonal:.6}", p0.total, p1.total);

    // p(1) should be at least the diagonal
    assert!(p1.total >= diagonal * 0.9,
        "p(1)={:.6} should be ≥ diagonal {diagonal:.6}", p1.total);
...
fn test_c_type_alpha() {
    ...
    assert!((p0.total - 0.99).abs() < 0.02, "p(0) ≈ 0.99: got {:.6}", p0.total);
...
fn test_a_type_alpha_zero_for_stabilizer() {
    ...
    eprintln!("A-type: p0_corr={:.8} p1_corr={:.8}", p0.s_correction + p0.h_correction, p1.s_correction + p1.h_correction);
    // The ca_correction is part of total but not separately exposed.
    // Just check total is unchanged from noiseless.
    assert!((p0.total - 1.0).abs() < 1e-6, "A on |0⟩ should not change p(0)");
