...
use pecos_lindblad::{
    synthesize_numerical, Gate, Lindbladian, Pauli1, PauliString, DEFAULT_N_STEPS,
};
...
fn paper_cx_rate(
    ...
    match label {
        ...
        "IZ" => f_anti_128 * (bd_r / omega) + (12.0 * theta + 8.0 * s2 + s4) / 64.0 * (bp_r / omega),
        ...
        "ZZ" => f_anti_128 * (bd_r / omega) + (12.0 * theta - 8.0 * s2 + s4) / 64.0 * (bp_r / omega),
        "XY" | "XZ" | "YY" | "YZ" | "ZX" => 0.0,
