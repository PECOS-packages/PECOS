...
//! p_meas, p_init }` (4 scalar probabilities). A proper integration requires
...
use pecos_lindblad::{
    synthesize_identity_1q, synthesize_numerical, Gate, Lindbladian, Pauli1,
    DEFAULT_N_STEPS,
};
...
fn lindblad_derived_noise_config_feeds_dem_stab_sim() {
    ...
    let pl_1q = synthesize_identity_1q(&Gate::identity(1, ad_plus_pd_1q(beta_down, beta_phi), tau_1q));
    ...
    assert!(total_1q > 0.0 && total_1q < 0.1, "1Q total rate out of range: {}", total_1q);
    assert!(total_2q > 0.0 && total_2q < 0.1, "2Q total rate out of range: {}", total_2q);
