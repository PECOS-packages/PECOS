...
fn model(entries: &[(&str, f64)]) -> PauliLindbladModel {
    let supports: Vec<_> = entries.iter().map(|(s, _)| PauliString::from_str(s).unwrap()).collect();
...
fn diagnose_flags_large_weight_2_residual() {
    ...
    assert!(diagnoses.iter().any(|m| m.contains("weight-2")), "expected weight-2 diagnosis in {:?}", diagnoses);
    assert!(diagnoses.iter().any(|m| m.contains("ZZ")), "expected ZZ mention in {:?}", diagnoses);
}
...
fn diagnose_quiet_when_models_agree() {
    ...
    assert!(diagnoses.is_empty(), "expected no diagnoses, got {:?}", diagnoses);
}
