...
use pecos_lindblad::{
    synthesize_identity_1q, synthesize_superop_time_dep, Gate, Pauli1, PauliString, RateFn,
    TimeDepLindbladian, DEFAULT_N_SLICES,
};
...
fn constant_rate_time_dep_matches_markovian() {
    ...
    let noise_td = TimeDepLindbladian::with_static_hamiltonian(
        d,
        matrix::zeros(d),
        vec![(z1q(), rate_fn)],
    );
