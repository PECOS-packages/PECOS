...
use pecos_lindblad::{
    synthesize_identity_1q, synthesize_numerical, Gate, DEFAULT_N_STEPS,
};
...
fn lindblad_rates_flow_through_per_gate_noise_spec() {
    ...
    // Bundle into a PerGateTypeNoise. Fallback for uncovered gate types
    // (e.g. PZ prep, MZ measure) falls back to a small uniform rate.
    let cfg = PerGateTypeNoise::from_fallback(NoiseConfig::new(0.0, 0.0, 1e-3, 1e-3))
...
fn to_noise_array_2q_preserves_paper_structure() {
    ...
    let pl = synthesize_numerical(
        &Gate::cx_theta(1.0, std::f64::consts::FRAC_PI_4, ad_pd_2q(100.0, 80.0, 100.0, 80.0)),
        DEFAULT_N_STEPS,
    );
...
fn to_noise_array_1q_panics_on_2q_model() {
    let pl = synthesize_numerical(
        &Gate::cx_theta(1.0, std::f64::consts::FRAC_PI_4, ad_pd_2q(100.0, 100.0, 80.0, 80.0)),
        DEFAULT_N_STEPS,
    );
