...
use pecos_lindblad::{
    synthesize_exact_unitary, Gate, Lindbladian, Pauli1, PauliString,
};
...
fn identity_2q_phase_noise_commuting_case() {
    ...
    assert_abs_diff_eq!(rate("IZ"), (delta_iz * tau_g).powi(2) / 4.0, epsilon = 1e-10);
    assert_abs_diff_eq!(rate("ZI"), (delta_zi * tau_g).powi(2) / 4.0, epsilon = 1e-10);
    assert_abs_diff_eq!(rate("ZZ"), (delta_zz * tau_g).powi(2) / 4.0, epsilon = 1e-10);
