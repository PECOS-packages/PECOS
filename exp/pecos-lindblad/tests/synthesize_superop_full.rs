...
use pecos_lindblad::{
    synthesize_exact_unitary, synthesize_numerical, synthesize_superop, Gate, Lindbladian, Pauli1,
    PauliString, DEFAULT_N_SLICES, DEFAULT_N_STEPS,
};
...
fn superop_handles_cx_mixed_ad_pd_plus_coherent_zz() {
    ...
    assert!(rate_mixed("IX") > 1e-8, "dissipative contribution should be present");
    assert!(rate_mixed("ZZ") > 1e-10, "coherent ZZ contribution should be present");
}
