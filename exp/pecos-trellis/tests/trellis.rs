...
fn merged_exact_config() -> TrellisConfig {
    TrellisConfig {
...
fn result_mass_map(result: &TrellisResult) -> BTreeMap<Vec<u64>, f64> {
    result
        .logical_masses
        .iter()
...
fn assert_result_semantics_bitwise_equal(left: &TrellisResult, right: &TrellisResult) {
...
fn indistinguishable_merging_matches_unmerged_and_original_brute_force() {
    ...
    for case_index in 0..24 {
        ...
        let mut unmerged = TrellisDecoder::from_sparse_dem(&dem, off_config).unwrap();
        let mut merged = TrellisDecoder::from_sparse_dem(&dem, on_config).unwrap();
...
fn merging_keeps_first_ordered_occurrence_and_deletes_later_copy() {
    ...
    .unwrap();
    let mut wrong_order = TrellisDecoder::from_sparse_dem(
        &moved_to_last,
        TrellisConfig {
            k: 1,
            delta: f64::INFINITY,
...
fn merge_requires_matching_observable_sets() {
    ...
    let mut decoder = TrellisDecoder::from_sparse_dem(&dem, merged_exact_config()).unwrap();
...
fn merge_is_default_off_and_preserves_the_unmerged_floating_path() {
    ...
    let mut default_decoder = TrellisDecoder::from_sparse_dem(&dem, default_config).unwrap();
    let mut explicit_off = TrellisDecoder::from_sparse_dem(&dem, exact_config()).unwrap();
    let mut hand_merged = TrellisDecoder::from_sparse_dem(&premerged, exact_config()).unwrap();
...
fn rejects_duplicate_detector_and_observable_indices() {
    ...
    let detector_error =
        TrellisDecoder::from_sparse_dem(&duplicate_detector, exact_config()).unwrap_err();
    ...
    let observable_error =
        TrellisDecoder::from_sparse_dem(&duplicate_observable, exact_config()).unwrap_err();
...
fn fails_loud_for_untouched_and_unachievable_syndromes() {
    ...
    let mut untouched = TrellisDecoder::from_sparse_dem(&untouched_dem, exact_config()).unwrap();
    ...
    let mut parity_locked =
        TrellisDecoder::from_sparse_dem(&parity_locked_dem, exact_config()).unwrap();
...
fn overpruning_can_remove_the_only_eventually_feasible_prefix() {
    ...
    let mut default_decoder =
        TrellisDecoder::from_sparse_dem(&dem, TrellisConfig::default()).unwrap();
    assert_eq!(
...
fn width_and_delta_pruning_can_change_the_logical_answer() {
    ...
    let mut exact = TrellisDecoder::from_sparse_dem(&dem, exact_config()).unwrap();
    assert!(exact.decode(&[1]).unwrap().predicted.is_zero());

    let mut greedy = TrellisDecoder::from_sparse_dem(
        &dem,
        TrellisConfig {
            k: 1,
            delta: f64::INFINITY,
    ...
    let mut delta_pruned = TrellisDecoder::from_sparse_dem(
        &dem,
        TrellisConfig {
            k: usize::MAX,
            delta: 0.1,
...
fn suffix_compatibility_changes_the_greedy_survivor() {
    ...
    let mut prefix_only = TrellisDecoder::from_sparse_dem(
        &dem,
        TrellisConfig {
            k: 1,
            delta: f64::INFINITY,
    ...
    .unwrap();
    let mut suffix_scored = TrellisDecoder::from_sparse_dem(
        &dem,
        TrellisConfig {
            k: 1,
            delta: f64::INFINITY,
...
fn decoding_is_bitwise_deterministic_including_a_tie() {
    ...
    let mut first_decoder = TrellisDecoder::from_sparse_dem(&dem, exact_config()).unwrap();
    let first = first_decoder.decode(&[1]).unwrap();
    let second = first_decoder.decode(&[1]).unwrap();
    let mut fresh_decoder = TrellisDecoder::from_sparse_dem(&dem, exact_config()).unwrap();
...
fn works_through_observable_decoder_trait_object() {
    ...
    let decoder = TrellisDecoder::from_sparse_dem(&dem, exact_config()).unwrap();
...
fn parses_a_stim_dem_string() {
    let dem_text = "\
        ...
        error(0.1) D0 D1 L0\n\
        error(0.2) D1\n";
    let mut decoder = TrellisDecoder::from_dem_str(dem_text, exact_config()).unwrap();
...
fn validates_probabilities_indices_order_and_pruning_configuration() {
    assert_eq!(
        TrellisConfig::default().score_alpha.to_bits(),
        0.8_f64.to_bits()
    );
    ...
    let mut zero_decoder = TrellisDecoder::from_sparse_dem(&zero_dem, exact_config()).unwrap();
    ...
    assert!(TrellisDecoder::from_sparse_dem(&invalid_zero_dem, exact_config()).is_err());

    let bad_detector = sparse_dem(vec![(0.1, vec![1], vec![])], 1, 0);
    assert!(TrellisDecoder::from_sparse_dem(&bad_detector, exact_config()).is_err());
    let bad_observable = sparse_dem(vec![(0.1, vec![], vec![1])], 0, 1);
    assert!(TrellisDecoder::from_sparse_dem(&bad_observable, exact_config()).is_err());
    ...
    for bad_order in [vec![0], vec![0, 0], vec![0, 2]] {
        ...
        assert!(TrellisDecoder::from_sparse_dem(&two_columns, config).is_err());
    }

    let k_error = TrellisDecoder::from_sparse_dem(
        &zero_dem,
        TrellisConfig {
            k: 0,
            ..TrellisConfig::default()
    ...
    let negative_delta_error = TrellisDecoder::from_sparse_dem(
        &zero_dem,
        TrellisConfig {
            delta: -0.1,
            ..TrellisConfig::default()
    ...
    let nan_delta_error = TrellisDecoder::from_sparse_dem(
        &zero_dem,
        TrellisConfig {
            delta: f64::NAN,
            ..TrellisConfig::default()
...
fn bp_scoring_is_bitwise_deterministic_across_reuse_and_fresh_construction() {
    ...
    let mut fresh_decoder = TrellisDecoder::from_sparse_dem(&dem, config).unwrap();
...
fn bp_flag_is_bitwise_inert_on_the_unpruned_fast_path() {
    ...
    for syndrome_mask in 0_u8..8 {
        let syndrome: Vec<u8> = (0..3)
            .map(|bit| u8::from(syndrome_mask & (1 << bit) != 0))
            .collect();
        let off_result = off.decode(&syndrome).unwrap();
        let on_result = on.decode(&syndrome).unwrap();

        assert_result_semantics_bitwise_equal(&off_result, &on_result);
        assert_eq!(
            off_result.bp_seconds.to_bits(),
            on_result.bp_seconds.to_bits()
        );
