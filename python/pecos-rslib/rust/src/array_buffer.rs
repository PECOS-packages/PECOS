...
pub fn ensure_f64_array(obj: &Bound<'_, PyAny>, param_name: &str) -> PyResult<ArrayD<f64>> {
    ...
    // Strategy 1: Try to extract via __array_interface__ (PECOS Array path)
    // This is the fast path - zero-copy for f64, single conversion for integers
    ...
    // NumPy's __array_interface__ is implemented as a data descriptor.
    //
    // We cannot use py.import("builtins").getattr("hasattr") because .getattr() has the
    // bug we're trying to work around. Instead, we use eval to directly access the function.
    let py = obj.py();
    let hasattr_fn = py.eval(c"hasattr", None, None)?;
    let has_array_interface: bool = hasattr_fn.call1((obj, "__array_interface__"))?.extract()?;

    if has_array_interface {
        // Try f64 first (zero-copy path)
        if let Ok(arr) = extract_f64_array(obj) {
            return Ok(arr);
        }

        // Try signed integer types and convert to f64
        #[allow(clippy::cast_precision_loss)] // Intentional: i64 to f64 for NumPy compatibility
        if let Ok(arr_i64) = extract_i64_array(obj) {
            return Ok(arr_i64.mapv(|x| x as f64));
        }
        if let Ok(arr_i32) = extract_i32_array(obj) {
            return Ok(arr_i32.mapv(f64::from));
        }
        if let Ok(arr_i16) = extract_i16_array(obj) {
            return Ok(arr_i16.mapv(f64::from));
        }
        if let Ok(arr_i8) = extract_i8_array(obj) {
            return Ok(arr_i8.mapv(f64::from));
        }

        // Try unsigned integer types and convert to f64
        #[allow(clippy::cast_precision_loss)] // Intentional: u64 to f64 for NumPy compatibility
        if let Ok(arr_u64) = extract_u64_array(obj) {
            return Ok(arr_u64.mapv(|x| x as f64));
        }
        if let Ok(arr_u32) = extract_u32_array(obj) {
            return Ok(arr_u32.mapv(f64::from));
        }
        if let Ok(arr_u16) = extract_u16_array(obj) {
            return Ok(arr_u16.mapv(f64::from));
        }
        if let Ok(arr_u8) = extract_u8_array(obj) {
            return Ok(arr_u8.mapv(f64::from));
        }

        // Try float32 and convert to f64
        if let Ok(arr_f32) = extract_f32_array(obj) {
            return Ok(arr_f32.mapv(f64::from));
        }

        // If we have __array_interface__ but extraction failed, provide helpful error
        // This catches complex arrays, bool arrays, and other unsupported types
        let array_iface = obj.getattr("__array_interface__")?;
        let interface = array_iface.cast::<pyo3::types::PyDict>()?;
        let typestr_obj = interface.get_item("typestr")?;

        if let Some(typestr) = typestr_obj {
            let typestr_value: String = typestr.extract()?;
            return Err(pyo3::exceptions::PyTypeError::new_err(format!(
                "Parameter '{param_name}': Unsupported array dtype '{typestr_value}'. \
                 Supported dtypes: float64, float32, int64, int32, int16, int8, uint64, uint32, uint16, uint8. \
                 Note: Complex and boolean arrays are not supported for real-valued numerical operations."
            )));
        }
    ...
    // This provides numpy-like convenience but requires allocation
    if let Ok(seq) = obj.clone().cast_into::<PySequence>() {
        let len = seq.len()?;
        if len == 0 {
            return Err(pyo3::exceptions::PyValueError::new_err(format!(
                "Parameter '{param_name}': Cannot create array from empty sequence"
            )));
        }
        ...
        for i in 0..len {
            ...
            if let Ok(val) = item.extract::<f64>() {
                ...
                let item_repr = item
                    ...
                    .unwrap_or_else(|_| format!("<{item_type}>"));
                return Err(pyo3::exceptions::PyTypeError::new_err(format!(
                    "Parameter '{param_name}': Cannot convert sequence element at index {i} to float64. \
                     Got {item_repr} of type '{item_type}'. All elements must be real numbers (int or float)."
                )));
            }
        }

        return ArrayD::from_shape_vec(vec![len], vec)
    ...
    let obj_repr = obj
        ...
        .unwrap_or_else(|_| format!("<{obj_type}>"));

    Err(pyo3::exceptions::PyTypeError::new_err(format!(
        "Parameter '{param_name}': Expected PECOS Array or numeric sequence (list/tuple), got {obj_repr} of type '{obj_type}'. \
         \n\nHint: Convert to PECOS Array using: pecos_rslib.array([...])"
    )))
}
