...
fn spec_family_name(spec: &pecos_decoders::DecoderSpec) -> &'static str {
    match spec {
        ...
        pecos_decoders::DecoderSpec::BpTrellis(_) => "bp_trellis",
        pecos_decoders::DecoderSpec::Frontier(_) => "frontier",
...
fn spec_repr(spec: &pecos_decoders::DecoderSpec) -> String {
    match spec {
        ...
        pecos_decoders::DecoderSpec::BpTrellis(config) => bp_trellis_repr(config),
        pecos_decoders::DecoderSpec::Frontier(config) => frontier_repr(config),
...
fn frontier(
    ...
    use pecos_decoders::spec::{FrontierColumnOrder, FrontierConfig, FrontierMetricMode};
    if k == 0 {
    ...
    Ok(PyDecoderSpec::new(pecos_decoders::DecoderSpec::Frontier(
        FrontierConfig {
            k,
            delta,
            score_alpha,
...
fn frontier_repr(config: &pecos_decoders::spec::FrontierConfig) -> String {
    use pecos_decoders::spec::{FrontierColumnOrder, FrontierConfig, FrontierMetricMode};
...
fn bp_trellis(
    ...
    use pecos_decoders::spec::{BpTrellisConfig, BpTrellisOrdering};
    if k == 0 {
        return Err(PyValueError::new_err("k must be at least 1"));
    }
    if delta.is_nan() || delta < 0.0 {
    ...
    Ok(PyDecoderSpec::new(pecos_decoders::DecoderSpec::BpTrellis(
        BpTrellisConfig {
            k,
            delta,
            score_alpha,
...
fn bp_trellis_repr(config: &pecos_decoders::spec::BpTrellisConfig) -> String {
    use pecos_decoders::spec::{BpTrellisConfig, BpTrellisOrdering};
