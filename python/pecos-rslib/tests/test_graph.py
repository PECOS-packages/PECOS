...
class TestMaxWeightMatching:
    ...
    def test_matching_chooses_heaviest_edge(self):
        ...
        graph.add_edge(n0, n1, 1.0)
        graph.add_edge(n1, n2, 10.0)  # Heaviest edge
    ...
    def test_matching_complex_graph(self):
        ...
        graph.add_edge(nodes[2], nodes[3], 8.0)
    ...
    def test_matching_max_cardinality_false(self):
        ...
        graph.add_edge(n0, n1, 100.0)
        graph.add_edge(n2, n3, 100.0)
...
class TestGraphUseCases:
    ...
    def test_mwpm_decoder_scenario(self):
        ...
        graph.add_edge(d0, d1, 10.0)  # Close together, high weight
        graph.add_edge(d2, d3, 10.0)  # Close together, high weight
        graph.add_edge(d0, d2, 2.0)  # Far apart, low weight
        graph.add_edge(d1, d3, 2.0)  # Far apart, low weight
...
class TestEdgeData:
    ...
    def test_add_edge_with_kwargs(self):
        """Test add_edge with keyword arguments."""
        graph = pc.graph.Graph()
        n0 = graph.add_node()
        n1 = graph.add_node()

        # Add edge with weight and custom attributes
        graph.add_edge(n0, n1, weight=5.0, data_path=[1, 2, 3], label="test")

        data = graph.get_edge_data(n0, n1)
        assert data is not None
        assert data["weight"] == 5.0
        assert data["data_path"] == [1, 2, 3]
        assert data["label"] == "test"

    def test_add_edge_kwargs_only(self):
        """Test add_edge with only kwargs (no positional weight)."""
        graph = pc.graph.Graph()
        n0 = graph.add_node()
        n1 = graph.add_node()

        # Add edge with only kwargs
        graph.add_edge(n0, n1, data_path=[4, 5], flag=True)

        data = graph.get_edge_data(n0, n1)
        assert data is not None
        assert data["weight"] == 1.0  # Default weight
        assert data["data_path"] == [4, 5]
        assert data["flag"] is True

    def test_add_edge_weight_kwarg(self):
        """Test add_edge with weight as keyword argument."""
        graph = pc.graph.Graph()
        n0 = graph.add_node()
        n1 = graph.add_node()

        # NetworkX style: weight as kwarg
        graph.add_edge(n0, n1, weight=7.5)
