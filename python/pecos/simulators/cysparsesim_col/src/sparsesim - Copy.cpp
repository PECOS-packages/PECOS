...
unsigned int State::nondeterministic_measure(const int_num& qubit,  
    ...
    // End choosing the smallest
    
    anticom_stabs.erase(removed_id);
    anticom_destabs.erase(removed_id);
    
    // Create removed row_x and row_z
    for (int_num q = 0; q < num_qubits; q++){
    
            if (stabs.col_z[q].count(removed_id)) {  // loop over removed_row_z
                ...
                for (const int_num& gen_id: anticom_destabs) {
                    if (destabs.col_z[q].count(gen_id)) {
                        destabs.col_z[q].erase(gen_id);
                    }   else {
                        destabs.col_z[q].insert(gen_id);
                    }
            ...
            if (stabs.col_x[q].count(removed_id)) { // loop over removed_row_x
                ...
                for (const int_num& gen_id: anticom_destabs) {
                    if (destabs.col_x[q].count(gen_id)) {
                        destabs.col_x[q].erase(gen_id);
                    }   else {
                        destabs.col_x[q].insert(gen_id);
                    }
            ...
            if (destabs.col_z[q].count(removed_id)) {  // loop over removed_row_z_destab
                destabs.col_z[q].erase(removed_id);
            }
            
            if (destabs.col_x[q].count(removed_id)) {  // loop over removed_row_x_destab
