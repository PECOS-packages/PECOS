...
def generate_color_transversal_source(d: int) -> str:
    ...
    lines = [
        ...
        "from guppylang.std.builtins import array, owned, output, comptime",
...
def _add_color_transversal_factory_functions(
    ...
    lines.extend(
        [
            ...
            '        output("init_synx_ctrl", init_syn_ctrl)',
            '        output("init_synx_tgt", init_syn_tgt)',
            "",
            ...
            '            output("synx_ctrl", syn_ctrl.synx)',
            '            output("synz_ctrl", syn_ctrl.synz)',
            '            output("synx_tgt", syn_tgt.synx)',
            '            output("synz_tgt", syn_tgt.synz)',
            "",
            "        # Final measurement",
            "        final_ctrl = measure_z_basis(ctrl)",
            "        final_tgt = measure_z_basis(tgt)",
            '        output("final_ctrl", final_ctrl)',
            '        output("final_tgt", final_tgt)',
            "",
            "        discard(ax_ctrl)",
            "        discard(az_ctrl)",
            "        discard(ax_tgt)",
...
def make_css_transversal_cnot(
    ...
    Example:
        ...
        >>> output = prog.emulator(n_qubits=18).stabilizer_sim().run()
    """
    ...
    if code_type == CSSCodeType.SURFACE:
        from pecos.guppy_gen.protocol_render import load_surface_protocol_module
        from pecos.qec.surface import SurfacePatch

        module = load_surface_protocol_module(SurfacePatch.create(distance=distance))
        return module["make_transversal_cx"](num_rounds, control_x=False)
...
def make_css_transversal_cnot_with_x(
    ...
    if code_type == CSSCodeType.SURFACE:
        ...
        module = load_surface_protocol_module(SurfacePatch.create(distance=distance))
        return module["make_transversal_cx"](num_rounds, control_x=True)
...
def get_transversal_num_qubits(code_type: CSSCodeType | str, distance: int) -> int:
    """Get peak live qubit count for transversal CNOT between two patches.
    ...
    Returns:
        Surface: two data arrays plus one round of ancillas (3 d^2 - 1).
        Color: 2 * num_data + 4 ancillas.
    """
    if isinstance(code_type, str):
        code_type = CSSCodeType(code_type)
