...
cdef class State:
    ...
    def measure(self, const int_num qubit, int forced_outcome = -1):
        return self._c_state.measure(qubit, forced_outcome)

    def measureX(self, const int_num qubit, int forced_outcome = -1):

        cdef unsigned int result

        self._c_state.hadamard(qubit)
        result = self._c_state.measure(qubit, forced_outcome)
        self._c_state.hadamard(qubit)
        return result

    def measureY(self, const int_num qubit, int forced_outcome = -1):
    ...
    cdef void initzero(self, const int_num qubit, int forced_outcome = -1):
        cdef unsigned int result
        result = self._c_state.measure(qubit, forced_outcome)
    ...
    cdef void initone(self, const int_num qubit, int forced_outcome = -1):
        cdef unsigned int result
        result = self._c_state.measure(qubit, forced_outcome)
    ...
    cdef void initplus(self, const int_num qubit, int forced_outcome = -1):
        self.initzero(qubit)
        self._c_state.hadamard(qubit, forced_outcome)

    cdef void initminus(self, const int_num qubit, int forced_outcome = -1):
        self.initone(qubit)
        self._c_state.hadamard(qubit, forced_outcome)

    cdef void initplusi(self, const int_num qubit, int forced_outcome = -1):
        self.initzero(qubit, forced_outcome)
        self._c_state.H5(qubit)

    cdef void initminusi(self, const int_num qubit, int forced_outcome = -1):
        self.initone(qubit, forced_outcome)
        self._c_state.H5(qubit)
