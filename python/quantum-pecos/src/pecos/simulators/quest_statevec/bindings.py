...
def _r2xxyyzz_decomposition(backend, qs, p):
...
def get_bindings(state: QuestStateVec) -> dict:
    ...
    return {
        ...
        "F1": lambda s, q, **p: (backend.sx_gate(q), backend.sz_gate(q))[-1] or None,  # F1 = F
        "F1d": lambda s, q, **p: (backend.szdg_gate(q), backend.sxdg_gate(q))[-1] or None,  # F1d = Fdg
        "F1dg": lambda s, q, **p: (backend.szdg_gate(q), backend.sxdg_gate(q))[-1] or None,  # F1dg = Fdg
        "F2": lambda s, q, **p: (backend.sxdg_gate(q), backend.sy_gate(q))[-1] or None,
        "F2d": lambda s, q, **p: (backend.sydg_gate(q), backend.sx_gate(q))[-1] or None,  # F2d = F2dg
        ...
        "F3d": lambda s, q, **p: (backend.szdg_gate(q), backend.sx_gate(q))[-1] or None,  # F3d = F3dg
        ...
        "F4d": lambda s, q, **p: (backend.sxdg_gate(q), backend.szdg_gate(q))[-1] or None,  # F4d = F4dg
        ...
        # S and T gates - use RZ implementation
        "S": lambda s, q, **p: backend.run_1q_gate("RZ", q, {"angle": 1.5707963267948966}),  # π/2
        "Sdg": lambda s, q, **p: backend.run_1q_gate("RZ", q, {"angle": -1.5707963267948966}),  # -π/2
        "SDG": lambda s, q, **p: backend.run_1q_gate("RZ", q, {"angle": -1.5707963267948966}),  # -π/2
        "SDAGGER": lambda s, q, **p: backend.run_1q_gate("RZ", q, {"angle": -1.5707963267948966}),
        ...
        "SXXdg": lambda s, qs, **p: (
            backend.run_1q_gate("X", qs[0] if isinstance(qs, (list, tuple)) else qs, None),
            backend.run_1q_gate("X", qs[1] if isinstance(qs, (list, tuple)) else qs, None),
            backend.sxx_gate(
                qs[0] if isinstance(qs, (list, tuple)) else qs,
                qs[1] if isinstance(qs, (list, tuple)) else qs,
            )
        ...
        "SYYdg": lambda s, qs, **p: (
            backend.run_1q_gate("Y", qs[0] if isinstance(qs, (list, tuple)) else qs, None),
            backend.run_1q_gate("Y", qs[1] if isinstance(qs, (list, tuple)) else qs, None),
            backend.syy_gate(
                qs[0] if isinstance(qs, (list, tuple)) else qs,
                qs[1] if isinstance(qs, (list, tuple)) else qs,
            )
        ...
        "SZZdg": lambda s, qs, **p: (
            backend.run_1q_gate("Z", qs[0] if isinstance(qs, (list, tuple)) else qs, None),
            backend.run_1q_gate("Z", qs[1] if isinstance(qs, (list, tuple)) else qs, None),
            backend.szz_gate(
                qs[0] if isinstance(qs, (list, tuple)) else qs,
                qs[1] if isinstance(qs, (list, tuple)) else qs,
            )
        ...
        "G2": lambda s, qs, **p: (
            ...
            backend.run_1q_gate("H", qs[1] if isinstance(qs, (list, tuple)) else qs, None),
            backend.run_2q_gate("CZ", tuple(qs) if isinstance(qs, list) else qs, None),
        )[-1] or None,  # G2 maps to same as G since StateVec does this
    }