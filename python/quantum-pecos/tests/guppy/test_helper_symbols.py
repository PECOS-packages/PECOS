...
def test_rename_every_symbol_occurrence(prefix: str, suffix: str) -> None:
    ...
    source = (
        ...
        f"define i64 @user() {{ %q = call i64 @{name}(i64 0, ptr null, ptr null) ret i64 %q }}\n"
    )
...
def test_identical_declarations_merge_after_attributes_and_whitespace() -> None:
    ...
    assert (
        normalize_pecos_helper_symbols(first + second)
        == first.replace(f"__hugr__.__main__.{_HELPER}.14", _HELPER) + "\n"
    )
...
@pytest.mark.parametrize(
    ...
    [
        "i32 (i64, ptr, ptr)",
        "i64 (i32, ptr, ptr)",
        "i64 (i64, ptr addrspace(1), ptr)",
        "i64 (i64, ptr, ptr, ...)",
        "i64 (i64 (i64)*, ptr, ptr)",
        "i64 (i64 *, ptr, ptr)",
        "i64 * (i64, ptr, ptr)",
        '%"return type" (i64, ptr, ptr)',
        'target("spirv.Event") (i64, ptr, ptr)',
    ],
)
...
def test_non_helpers_are_untouched() -> None:
    source = "declare i64 @__hugr__.other.14(i64)\n"
...
def test_quoted_symbol_with_guppy_scope_is_renamed() -> None:
    ...
    source = (
        f"declare i64 @{name}(i64, ptr, ptr)\n"
        f"define i64 @user() {{ %q = call i64 @{name}(i64 0, ptr null, ptr null) ret i64 %q }}\n"
    )
    assert normalize_pecos_helper_symbols(source) == source.replace(name, _HELPER)
...
def test_multiline_helper_definition_is_rejected() -> None:
    helper = "pecos_qis_runtime_barrier_qubit_hugr"
    source = f"define i64\n@__hugr__.m.{helper}.1(i64 %q) {{ ret i64 %q }}"
    with pytest.raises(ValueError, match=f"{helper}.*defined.*line 2"):
        normalize_pecos_helper_symbols(source)


def test_multiline_declaration_is_validated() -> None:
    source = f"declare\n noundef i64\n @__hugr__.{_HELPER}.1(\n i64, ; qubit\n ptr,\n ptr\n) nounwind\n"
    assert normalize_pecos_helper_symbols(source) == source.replace(f"__hugr__.{_HELPER}.1", _HELPER)


@pytest.mark.parametrize("conflicting", [False, True])
def test_two_declarations_on_one_line(conflicting: bool) -> None:
    first = f"declare i64 @{_HELPER}(i64, ptr, ptr)"
    second = f"declare {'void' if conflicting else 'i64'} @__hugr__.{_HELPER}.2(i64, ptr, ptr)"
    if conflicting:
        with pytest.raises(ValueError, match=f"{_HELPER}.*conflicting"):
            normalize_pecos_helper_symbols(first + " " + second)
    else:
        assert normalize_pecos_helper_symbols(first + " " + second) == first + " "


@pytest.mark.parametrize("keyword", ["declare", "define"])
def test_hex_escaped_public_helper_is_validated(keyword: str) -> None:
    helper = "pecos_qis_runtime_barrier_qubit_hugr"
    source = f'{keyword} void @"\\70{helper[1:]}"(i64)' + (" { ret void }" if keyword == "define" else "")
    with pytest.raises(ValueError, match=helper):
        normalize_pecos_helper_symbols(source)


def test_hex_escaped_private_helper_is_renamed_everywhere() -> None:
    name = f'"__hugr__\\2Em.\\70{_HELPER[1:]}.10"'
    source = (
        f"declare i64 @{name}(i64, ptr, ptr)\n"
        f"define i64 @user(i64 %q) {{ %r = call i64 @{name}(i64 %q, ptr null, ptr null) ret i64 %r }}\n"
    )
    assert normalize_pecos_helper_symbols(source) == source.replace(name, _HELPER)


def test_duplicate_removal_preserves_trailing_comment_and_next_declaration() -> None:
    first = f"declare i64 @{_HELPER}(i64, ptr, ptr)\n"
    second = f"declare i64 @__hugr__.{_HELPER}.2(i64, ptr, ptr) nounwind ; keep me\n"
    following = "declare void @other()\n"
    assert normalize_pecos_helper_symbols(first + second + following) == first + " ; keep me\n" + following


def test_duplicate_removal_preserves_internal_comment() -> None:
    first = f"declare i64 @{_HELPER}(i64, ptr, ptr)\n"
    second = f"declare i64 @__hugr__.{_HELPER}.2(i64, ; keep me\n ptr, ptr)\n"
    assert normalize_pecos_helper_symbols(first + second) == first + "; keep me\n\n"
...
@pytest.mark.parametrize(
    ...
    [
        "@{helper} = global i64 0",
        "@other = global ptr @{helper}",
        "@other = alias i64 (i64, ptr, ptr), ptr @{helper}",
        "!0 = !{{ptr @{helper}}}",
        "define void @other() personality ptr @{helper} {{ ret void }}",
        "define void @other() prefix {{ptr}} {{ptr @{helper}}} {{ ret void }}",
        "@{helper}",
    ],
)
def test_helper_outside_declaration_or_instruction_is_rejected(source: str) -> None:
    with pytest.raises(ValueError, match=f"{_HELPER}.*line 2"):
        normalize_pecos_helper_symbols("; first line\n" + source.format(helper=_HELPER))


def test_instruction_operands_are_allowed_across_lines() -> None:
    source = (
        f"declare i64 @{_HELPER}(i64, ptr, ptr)\n"
        f"define ptr @other() {{ %f = bitcast ptr\n @{_HELPER} to ptr ret ptr %f }}\n"
    )
    assert normalize_pecos_helper_symbols(source) == source


def test_unrecognized_namer_suffix_is_rejected() -> None:
    symbol = "__hugr__.__main__.pecos_qis_runtime_barrier_qubit_hugr.10.7"
    with pytest.raises(ValueError, match="Unrecognized") as error:
        normalize_pecos_helper_symbols(f"declare i64 @{symbol}(i64)")
    assert symbol in str(error.value)


def test_non_numeric_helper_suffix_is_rejected() -> None:
    symbol = f"__hugr__.{_HELPER}.user"
    with pytest.raises(ValueError, match="Unrecognized") as error:
        normalize_pecos_helper_symbols(f"declare i64 @{symbol}(i64)")
    assert symbol in str(error.value)


def test_conflicting_return_pointer_types_report_complete_signatures() -> None:
    first = f"declare i64 * @{_HELPER}(i64, ptr, ptr)\n"
    second = f"declare i32 * @__hugr__.{_HELPER}.1(i64, ptr, ptr)\n"
    with pytest.raises(ValueError, match="conflicting") as error:
        normalize_pecos_helper_symbols(first + second)
    assert "i64 * (i64, ptr, ptr)" in str(error.value)
    assert "i32 * (i64, ptr, ptr)" in str(error.value)
