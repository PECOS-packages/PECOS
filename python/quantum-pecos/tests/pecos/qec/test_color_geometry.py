...
class TestGenerate488Layout:
    ...
    def test_polygon_contains_color(self):
...
class TestColorCode488:
    ...
    def test_get_stabilizers_by_color(self):
        ...
        assert len(red) + len(green) + len(blue) == code.n_stabilizers
    ...
    def test_parity_matrix_shape(self):
        """Parity matrix should have shape (n_stab, n_data)."""
        code = ColorCode488.create(distance=3)
        H = code.get_parity_matrix()
        assert H.shape == (code.n_stabilizers, code.n_data)
...
class TestColorCode488Builder:
    ...
    def test_builder_requires_distance(self):
...
class TestColorCodeStabilizer:
    ...
    def test_stabilizer_is_frozen(self):
