...
"""Regression tests for RXY1Q gate operations with conditional execution.

These tests verify correct behavior of RXY1Q gates, particularly in conditional
contexts and with various angle combinations.

Background:
    GitHub issue #81 revealed a bug where RXY1Q gates with theta=2.0 were incorrectly
    ...
    This caused conditional RXY1Q gates with angles summing to 4π to not cancel
...
def test_rxy1q_angles_summing_to_4pi_cancel() -> None:
    """Test that two RXY1Q gates with angles summing to 4π cancel completely.
...
def test_rxy1q_theta_2pi_classified_as_identity() -> None:
    """Test that RXY1Q with theta=2π is correctly classified as identity.
    ...
    # RXY1Q with theta=2π should be identity, leaving qubit in |0⟩
    zeros = meas_bit(results["a"]).count("0")
    assert zeros == 100, f"RXY1Q(2π, 0) should be identity, but got {100-zeros} non-zero results"


def test_rxy1q_theta_2_not_identity() -> None:
    """Test that RXY1Q with theta=2.0 is NOT classified as identity.
    ...
    # RXY1Q(2.0, 0) should rotate the qubit, not be identity
    # We expect a mix of 0s and 1s, not all 0s
    ones = meas_bit(results["a"]).count("1")

    # With a 2 radian rotation, we should NOT get all zeros
    assert ones > 0, "RXY1Q(2.0, 0) should not be identity - expected some 1s in results"


def test_rxy1q_angles_summing_to_2pi_return_to_identity() -> None:
    """Test that RXY1Q gates with angles summing to 2π return to identity.

    Two RXY1Q gates with theta values summing to 2π should cancel (identity up to global phase).
    """
    phir = """{
    "format": "PHIR/JSON",
    "version": "0.1.0",
    ...
    # RXY1Q(π, 0) + RXY1Q(π, 0) = rotation by 2π = identity (up to global phase)
    # Qubit should remain in |0⟩
    zeros = meas_bit(results["a"]).count("0")
    assert zeros == 100, f"Expected all measurements to be 0 (2π rotation = identity), but got {100-zeros} ones"


def test_conditional_rxy1q_with_false_condition() -> None:
    """Test that conditional RXY1Q is not executed when condition is false."""
    phir = """{
    "format": "PHIR/JSON",
    "version": "0.1.0",
    "metadata": {"source": "test"},
    ...
    # Condition is false (t[0]=0, not 1), so RXY1Q should not execute
    # Qubit should remain in |0⟩
    zeros = meas_bit(results["a"]).count("0")
    assert zeros == 100, f"Expected all zeros (RXY1Q not executed), but got {100-zeros} ones"


def test_rxy1q_alternative_angles_summing_to_4pi() -> None:
