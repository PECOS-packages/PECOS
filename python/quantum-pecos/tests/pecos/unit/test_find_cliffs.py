...
def test_rxy1q_identity_recognition_is_symmetric_around_tau_multiples(theta, phi) -> None:
    assert rxy1q2cliff(theta, phi, atol=1e-9) == "I"
...
def test_identity_recognition_uses_only_requested_absolute_tolerance(
    ...
    assert rxy1q2cliff(theta, 0.37, atol=atol) == expected
    assert rz2cliff(theta, atol=atol) == expected


@pytest.mark.parametrize("atol", [1e-12, 1e-9])
@pytest.mark.parametrize("direction", [-1, 1])
def test_rxy1q_table_matching_uses_only_requested_absolute_tolerance(
    atol,
    direction,
) -> None:
    ...
    assert rxy1q2cliff(theta + inside_offset, phi, atol=atol) == "SY"
    assert rxy1q2cliff(theta, phi + inside_offset, atol=atol) == "SY"
    assert rxy1q2cliff(theta + outside_offset, phi, atol=atol) is False
    assert rxy1q2cliff(theta, phi + outside_offset, atol=atol) is False
...
@pytest.mark.parametrize(
    ("angles", "expected"),
    list(rxy1q_ang2str.items()),
)
def test_rxy1q_matrix_fallback_matches_conversion_table(angles, expected) -> None:
    theta, phi = angles
    assert (
        rxy1q2cliff(
            theta,
...
def test_matrix_fallback_propagates_tolerance() -> None:
    ...
    assert rxy1q2cliff(theta, 0.0, atol=1e-9, use_conv_table=False) == "X"
    assert rxy1q2cliff(theta, 0.0, atol=1e-12, use_conv_table=False) is False
