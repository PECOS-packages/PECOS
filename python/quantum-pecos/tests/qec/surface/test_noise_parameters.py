...
def test_every_field_has_a_mechanical_fluent_setter() -> None:
    ...
    for field_name in field_names - _FAMILY_MODEL_FIELDS:
...
def test_idle_family_rate_and_model_set_together() -> None:
    ...
    assert noise.p_idle_z_linear_rate == pytest.approx(0.01)
    assert noise.p_idle_x_linear_rate in (None, 0.0)
...
def test_each_idle_family_round_trips_through_runtime_conversion() -> None:
    ...
    assert linear.for_runtime_idle_time_units(time_units_per_second=10.0).p_idle_z_linear_rate == pytest.approx(0.03)
    converted_sine = sine.for_runtime_idle_time_units(time_units_per_second=10.0)
    assert converted_sine.p_idle_x_quadratic_sine_rate == pytest.approx(0.02)
...
def test_legacy_idle_alias_setter_warns_but_family_setter_does_not() -> None:
    with pytest.warns(DeprecationWarning, match="p_idle_linear_rate"):
        NoiseParameters().with_p_idle_linear_rate(0.01)

    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        NoiseParameters().with_p_idle_linear(0.01)
