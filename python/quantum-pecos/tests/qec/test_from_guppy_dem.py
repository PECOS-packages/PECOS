...
@pytest.mark.parametrize(
    "idle_noise",
    [
        pytest.param({"p_idle": 0.01}, id="uniform"),
        pytest.param({"p_idle_z_linear_rate": 0.01}, id="axis-rate"),
        pytest.param({"t1": 100.0}, id="t1"),
        pytest.param({"t2": 100.0}, id="t2"),
    ],
)
def test_from_guppy_rejects_idle_noise_without_idle_gates(idle_noise: dict[str, float]) -> None:
    with pytest.raises(ValueError, match=r"idle-noise parameters have no idle gates"):
        _two_qubit_dem(**idle_noise)
