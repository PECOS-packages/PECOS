...
class TestStabilizerBasic:
    """Basic functionality tests for the stabilizer plugin."""
...
class TestStabilizerAngleValidation:
    ...
    def test_invalid_angle_threshold(self) -> None:
        ...
        with pytest.raises(ValueError, match="greater than zero"):
            StabilizerPlugin(angle_threshold=0)

        with pytest.raises(ValueError, match="greater than zero"):
            StabilizerPlugin(angle_threshold=-0.1)

    def test_valid_angle_threshold(self) -> None:
        """Test that valid angle thresholds are accepted."""
        plugin = StabilizerPlugin(angle_threshold=1e-6)
        assert plugin.angle_threshold == 1e-6

        plugin = StabilizerPlugin(angle_threshold=0.1)
        assert plugin.angle_threshold == 0.1


class TestStabilizerPlugin:
    ...
    def test_init_args(self) -> None:
        ...
        plugin = StabilizerPlugin(angle_threshold=1e-5)
