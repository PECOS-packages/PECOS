...
mod tests {
    ...
    fn rpp_batch_rotates_both_qubits_and_checks_targets() {
        ...
        let mut sim = Arc::new(StateVecSimulatorFactory).init(3, &args).unwrap();
    ...
    #[test]
    fn rpp_batch_preserves_arbitrary_axis_phase() {
        use super::*;
        let theta: f64 = -0.73;
        let phi: f64 = 0.41;
        let mut sim = Arc::new(StateVecSimulatorFactory)
            .init(2, &[String::new()])
            .unwrap();
        sim.handle_operations(BatchOperation::error_model(vec![Operation::RPPGate {
            qubit_id_1: 0,
        ...
        .unwrap();
        // Starting from |00>, the |11> amplitude is -i sin(theta/2)
        // times exp(2i phi), so both signs and both angles matter here.
        let zero = sim.simulator.get_amplitude(0);
        let one = sim.simulator.get_amplitude(3);
        assert!((zero.re - (theta / 2.0).cos()).abs() < 1e-12);
        assert!(zero.im.abs() < 1e-12);
        assert!((one.re - (theta / 2.0).sin() * (2.0 * phi).sin()).abs() < 1e-12);
        assert!((one.im + (theta / 2.0).sin() * (2.0 * phi).cos()).abs() < 1e-12);
        assert!(sim.simulator.get_amplitude(1).norm() < 1e-12);
        assert!(sim.simulator.get_amplitude(2).norm() < 1e-12);
    }
}
