...
from pecos.slr import SlrConverter, Main, QReg, CReg, Permute
...
def test_quantum_permutation_qir(quantum_permutation_program):
    ...
    h_calls = re.findall(r'call void @__quantum__qis__h__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    cnot_calls = re.findall(r'call void @__quantum__qis__cnot__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\), %Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
...
def test_permutation_with_bell_circuit_qir():
    ...
    h_calls = re.findall(r'call void @__quantum__qis__h__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    cx_calls = re.findall(r'call void @__quantum__qis__cnot__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\), %Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
...
def test_comprehensive_qir_verification():
    ...
    h_calls = re.findall(r'call void @__quantum__qis__h__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    x_calls = re.findall(r'call void @__quantum__qis__x__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    y_calls = re.findall(r'call void @__quantum__qis__y__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    z_calls = re.findall(r'call void @__quantum__qis__z__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    cx_calls = re.findall(r'call void @__quantum__qis__cnot__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\), %Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    mz_to_creg_calls = re.findall(r'call void @mz_to_creg_bit\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\), i1\* %(\w+), i64 (\d+)\)', qir)
    ...
    if len(h_calls) >= 1 and len(x_calls) >= 1 and len(y_calls) >= 1 and len(z_calls) >= 1:
        ...
        print(f"Inferred qubit allocation:")
        ...
        if len(h_calls) >= 2 and len(x_calls) >= 2:
            assert int(h_calls[1]) == original_b0, f"Second H gate should be applied to original b[0] (physical qubit {original_b0}), but was applied to physical qubit {h_calls[1]}"
            assert int(x_calls[1]) == original_a0, f"Second X gate should be applied to original a[0] (physical qubit {original_a0}), but was applied to physical qubit {x_calls[1]}"
        ...
        if len(y_calls) >= 2 and len(z_calls) >= 2:
            assert int(y_calls[1]) == original_b1, f"Second Y gate should be applied to original b[1] (physical qubit {original_b1}), but was applied to physical qubit {y_calls[1]}"
            assert int(z_calls[1]) == original_a1, f"Second Z gate should be applied to original a[1] (physical qubit {original_a1}), but was applied to physical qubit {z_calls[1]}"
        
        # The CX gate should be from "original b[0]" to "original a[1]"
        if len(cx_calls) >= 1:
            cx_control, cx_target = map(int, cx_calls[0])
            assert cx_control == original_b0, f"CX control should be original b[0] (physical qubit {original_b0}), but was physical qubit {cx_control}"
            assert cx_target == original_a1, f"CX target should be original a[1] (physical qubit {original_a1}), but was physical qubit {cx_target}"
        ...
        if len(mz_to_creg_calls) >= 2:
            ...
            assert b0_to_m0, f"Expected measurement from original b[0] (physical qubit {original_b0}) to m[0], but found measurements: {mz_to_creg_calls}"
            assert a1_to_n0, f"Expected measurement from original a[1] (physical qubit {original_a1}) to n[0], but found measurements: {mz_to_creg_calls}"
...
def test_rotation_gates_with_permutation():
    ...
    # Extract all gate operations to track qubit allocation
    rx_calls = re.findall(r'call void @__quantum__qis__rx__body\(double (0x[0-9a-f]+), %Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    ry_calls = re.findall(r'call void @__quantum__qis__ry__body\(double (0x[0-9a-f]+), %Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    rz_calls = re.findall(r'call void @__quantum__qis__rz__body\(double (0x[0-9a-f]+), %Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    s_calls = re.findall(r'call void @__quantum__qis__s__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    t_calls = re.findall(r'call void @__quantum__qis__t__body\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    tdg_calls = re.findall(r'call void @__quantum__qis__t__adj\(%Qubit\* inttoptr \(i64 (\d+) to %Qubit\*\)\)', qir)
    ...
    if len(rx_calls) >= 1 and len(ry_calls) >= 1 and len(rz_calls) >= 1 and len(s_calls) >= 1:
        ...
        if len(rx_calls) >= 2 and len(ry_calls) >= 2:
            assert int(rx_calls[1][1]) == original_b0, f"Second Rx gate should be applied to original b[0] (physical qubit {original_b0}), but was applied to physical qubit {rx_calls[1][1]}"
            assert int(ry_calls[1][1]) == original_a0, f"Second Ry gate should be applied to original a[0] (physical qubit {original_a0}), but was applied to physical qubit {ry_calls[1][1]}"
        
        if len(t_calls) >= 1 and len(tdg_calls) >= 1:
            assert int(t_calls[0]) == original_a1, f"T gate should be applied to original a[1] (physical qubit {original_a1}), but was applied to physical qubit {t_calls[0]}"
            assert int(tdg_calls[0]) == original_b1, f"Tdg gate should be applied to original b[1] (physical qubit {original_b1}), but was applied to physical qubit {tdg_calls[0]}" 