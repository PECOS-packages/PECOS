...
class TestQulacsGateBindings:
    ...
    def test_single_qubit_initialization(self):
        ...
        assert np.allclose(sim.vector, expected_before), f"Initial state incorrect: {sim.vector}"
        
        # Reset qubit 1 to |0⟩ (should be no change since it's already |0⟩)
        sim.bindings["init |0>"](sim, 1)
        assert np.allclose(sim.vector, expected_before), f"Reset qubit 1 to |0⟩ changed other qubits: {sim.vector}"
        ...
        assert np.allclose(sim.vector, expected_after_init_one), f"Init qubit 1 to |1⟩ incorrect: {sim.vector}"
        ...
        assert np.allclose(sim.vector, expected_after_reset_0), f"Reset qubit 0 to |0⟩ incorrect: {sim.vector}"
