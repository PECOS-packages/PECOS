...
# Both wrap scripts/ci/cache_guard.py, which parses the workflow as YAML and
# looks only at the owning step's own `if:` or `with:` mapping.
# cache_guard_ok: exit 0 when `<key>` is a literal false or a push-to-trusted-
# branch predicate, 1 otherwise, 2 when the step has no such input.
cache_guard_ok() {
    uv run --frozen python scripts/ci/cache_guard.py "$1" "$2" "$3"
}

# step_input_value: print the owning step's `<key>` scalar; exit 2 when absent.
step_input_value() {
    uv run --frozen python scripts/ci/cache_guard.py "$1" "$2" "$3" --value
}
